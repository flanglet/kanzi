...
/**
 * The {@code BucketSort} class provides an implementation of the bucket sort algorithm for sorting
 * integers and bytes. Bucket sort is a simple and efficient sorting algorithm that works by
 * distributing elements into a number of buckets, then sorting the individual buckets. This
 * implementation is optimized to handle small integer and byte values.
 *
 * <p>
 * It is a simplified form of radix sort with buckets of width one, making it efficient for small
 * integers (up to 0xFFFF).
 * </p>
 * <p>
...
public class BucketSort implements IntSorter, ByteSorter {
  ...
  /**
   * Constructs a {@code BucketSort} object using the default bucket size for byte values (0 to
   * 255).
   */
  public BucketSort() {
  ...
  /**
   * Constructs a {@code BucketSort} object with a custom bucket size determined by the logarithm of
   * the maximum value.
   *
   * @param logMaxValue the logarithm (base 2) of the maximum value to be sorted. Must be between 2
   *        and 16 (inclusive).
   ...
   *
   * <p>
   * The sorting works by counting the frequency of each integer in the input array, then placing
   * the integers back into the array in sorted order.
   * </p>
   *
   ...
   * @return {@code true} if the sorting was successful; {@code false} if there were invalid
   *         parameters (e.g., out-of-bounds indices or invalid length).
   */
  @Override
   ...
   *
   * <p>
   * This method behaves similarly to the integer sort method, but operates on byte values (0 to
   * 255).
   * </p>
   *
   ...
   * @param len the length of the portion of the array to be sorted.
   * @return {@code true} if the sorting was successful; {@code false} if there were invalid
   *         parameters ( out-of-bounds indices or invalid length).
   */
  @Override
