...
/**
 * A default implementation of the {@link OutputBitStream} interface that writes
 * bits to an output stream. This class buffers output data for efficient
 * bit-level writing and handles various edge cases.
 * <p>
 * The output stream is written to using a byte buffer. The buffer size must be
 * a multiple of 8 and at least 1024 bytes.
 * </p>
 */
public final class DefaultOutputBitStream implements OutputBitStream {
    ...
    /**
     * Constructs a DefaultOutputBitStream with the specified output stream and
     * buffer size.
     *
     * @param os
     *            the OutputStream to write bits to
     * @param bufferSize
     *            the size of the buffer (must be at least 1024 and a multiple of 8)
     * @throws NullPointerException
     *             if the provided OutputStream is null
     * @throws IllegalArgumentException
     *             if the buffer size is invalid
     */
    ...
    /**
     * Writes the least significant bit of the specified integer to the output
     * stream.
     *
     * @param bit
     *            the bit value to write (should be either 0 or 1)
    ...
    /**
     * Writes a specified number of bits from the provided long value to the output
     * stream.
     *
     * @param value
     *            the long value containing the bits to write
     * @param count
     *            the number of bits to write (must be in the range [1..64])
    ...
    /**
     * Writes the specified number of bits from the provided byte array to the
     * output stream.
     *
     * @param bits
    ...
    public int writeBits(byte[] bits, int start, int count) {
        ...
        if ((count >> 3) > bits.length - start)
            throw new IllegalArgumentException(
                    "Invalid length: " + count + " (must be in [1.." + (((long) (bits.length - start)) << 3) + "])");
    ...
    /**
     * Pushes the current 64 bits into the buffer. This method is called when the
    ...
    /**
     * Returns the total number of bits written so far, including both bits flushed
     * to the output stream and bits currently held in memory.
     *
