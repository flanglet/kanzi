...
/**
 * Implementation of a {@link java.io.InputStream} that decompresses a stream compressed using the
 * {@link CompressedOutputStream}. This class reads and decompresses data using a custom compression
 * algorithm. It supports parallel processing, block-based reading, and uses listeners for status
 * updates.
 *
 * The decompression is done in blocks and uses a custom input bitstream that allows efficient
 * bit-level reading.
 *
 * <p>
...
public class CompressedInputStream extends InputStream {
  ...
  /**
   * Creates a new {@link CompressedInputStream} from the specified input stream and context map.
   * Uses the default buffer size for reading data.
   *
  ...
  /**
   * Creates a new {@link CompressedInputStream} from the specified custom input bitstream and
   * context map.
   *
  ...
  public CompressedInputStream(InputBitStream ibs, Map<String, Object> ctx) {
    ...
    if (this.headless == true) {
      ...
      if ((this.blockSize < MIN_BITSTREAM_BLOCK_SIZE)
          || (this.blockSize > MAX_BITSTREAM_BLOCK_SIZE))
  ...
  /**
   * Reads and processes the header of the compressed bitstream, initializing necessary internal
   * states.
   *
  ...
  protected void readHeader() throws IOException {
    ...
    try {
      ...
      throw new KanziIOException(
          "Invalid bitstream, unknown entropy codec type: " + this.entropyType,
   ...
   * <code>array</code>. The number of bytes actually read is returned as an integer. This method
   * blocks until input data is available, end of file is detected, or an exception is thrown.
   *
   * <p>
   * If the length of <code>array</code> is zero, then no bytes are read and <code>0</code> is
   ...
   * at least one byte is read and stored into <code>array</code>.
   *
   * <p>
   * The first byte read is stored into element <code>array[0]</code>, the next one into
   * <code>array[1]</code>, and so on. The number of bytes read is, at most, equal to the length of
   * <code>array</code>. Let <i>k</i> be the number of bytes actually read; these bytes will be
   * stored in elements <code>array[0]</code> through <code>array[</code><i>k</i><code>-1]</code>,
   * leaving elements <code>array[</code><i>k</i><code>]</code> through
   * <code>array[array.length-1]</code> unaffected.
   *
   * <p>
   * The <code>read(array)</code> method for class <code>InputStream</code> has the same effect as:
   * 
   * <pre>
   ...
   *            the file, if the input stream has been closed, or if some other I/O error occurs.
  ...
  private long processBlock() throws IOException {
    ...
    try {
      ...
      final int blkSize =
          Math.max(this.blockSize + EXTRA_BUFFER_SIZE, this.blockSize + (this.blockSize >> 4));
      ...
      while (true) {
        ...
        for (int taskId = 0; taskId < nbTasks; taskId++) {
          ...
          Callable<Status> task =
              new DecodingTask(this.buffers[taskId], this.buffers[this.jobs + taskId], blkSize,
                  this.transformType, this.entropyType, firstBlockId + taskId + 1, this.ibs,
        ...
        for (Status res : results) {
          ...
          if (blockListeners.length > 0) {
            ...
            Event evt = new Event(Event.Type.AFTER_TRANSFORM, res.blockId, res.decoded,
                res.checksum, hashType, res.completionTime);
  ...
  /**
   * Returns the total number of bytes read so far from the compressed stream. The method calculates
   * the total number of bytes based on the number of bits read by the input bitstream, converting
   * it to bytes.
   * 
   ...
   * method is called with the provided event. Any exceptions thrown by the listeners are caught and
   * ignored to prevent failure of the notification process.
   * 
  ...
  /**
   * A task that decodes a single block of compressed data. This class allows parallel processing of
   * multiple blocks. Each task handles the transformation and entropy decoding of a block
   * independently. The task is executed asynchronously using an {@link ExecutorService}.
   * 
   * <p>
   * The task is constructed with the necessary buffers, configuration, and context for processing
   * the block. The actual decoding logic is executed by the {@link #call()} method.
   * </p>
   * 
   * @see SliceByteArray
   * @see InputBitStream
  ...
  static class DecodingTask implements Callable<Status> {
     ...
     */
    DecodingTask(SliceByteArray iBuffer, SliceByteArray oBuffer, int blockSize, long transformType,
        int entropyType, int blockId, InputBitStream ibs, XXHash32 hasher32, XXHash64 hasher64,
    ...
    /**
     * The entry point of the decoding task. This method is invoked by the executor service and is
     * responsible for decoding the block. The task processes the input data in the buffer and
     * applies the necessary transformations and entropy decoding.
     *
     * <p>
     * The actual decoding logic is not implemented in this class. The {@link #call()} method should
     * be extended or modified to implement the block decoding functionality.
     * </p>
     *
    ...
    public Status call() throws Exception {
      return this.decodeBlock(this.data, this.buffer, this.transformType, this.entropyType,
          this.blockId);
    }
    ...
    private Status decodeBlock(SliceByteArray data, SliceByteArray buffer, long blockTransformType,
      ...
      try {
        ...
        if (this.listeners.length > 0) {
          ...
          if (v >= 5) {
            String bsf =
                String.format("%8s", Integer.toBinaryString(skipFlags & 0xFF)).replace(" ", "0");
            String msg =
        ...
        if (this.listeners.length > 0) {
          ...
          Event evt1 = new Event(Event.Type.AFTER_ENTROPY, currentBlockId, preTransformLength,
              checksum1, hashType);

          notifyListeners(this.listeners, evt1);

          // Notify before transform (block size after entropy decoding)
          Event evt2 = new Event(Event.Type.BEFORE_TRANSFORM, currentBlockId, preTransformLength,
              checksum1, hashType);
  ...
  /**
   * Represents the status of a decoding task. The status includes information about the decoded
   * block, including the block ID, the number of decoded bytes, the checksum, any errors that
   * occurred, a message, and the completion time.
   *
   * <p>
  ...
  static class Status {
    ...
    /**
     * Constructs a new {@link Status} object with the specified values, including an optional
     * "skipped" flag indicating whether the block was skipped.
     *
     * @param data The decompressed data.
     * @param blockId The block ID of the decoded block.
     * @param decoded The number of bytes decoded.
