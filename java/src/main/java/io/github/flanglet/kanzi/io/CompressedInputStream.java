 ...
 *
 * The decompression is done in blocks and uses a custom input bitstream that
 * allows efficient bit-level reading.
 *
 * <p>
 * This class is thread-safe and can be used with multiple concurrent tasks for
 * decompression. The compression format is identified by a unique bitstream
 * type ("KANZ") and supports multiple versions and transformations.
 * </p>
 *
...
public class CompressedInputStream extends InputStream {
    ...
    /**
     * Context map containing additional configuration options for the
     * decompression.
     */
     ...
     * @param ctx
     *            A map containing additional context information for decompression.
     */
    ...
    /**
     * Creates a new {@link CompressedInputStream} from the specified custom input
     * bitstream and context map.
     *
     * @param ibs
     *            The input bitstream to read compressed data from.
     * @param ctx
     *            A map containing additional context information for decompression.
     */
    ...
    /**
     * Reads and processes the header of the compressed bitstream, initializing
     * necessary internal states.
     *
     * @throws IOException
     *             If an I/O error occurs during header reading.
     */
    protected void readHeader() throws IOException {
        ...
        // Sanity check
        if (bsVersion > BITSTREAM_FORMAT_VERSION)
            throw new KanziIOException("Invalid bitstream, cannot read this version of the stream: " + bsVersion,
        ...
        if (bsVersion >= 6) {
            ...
            if (chkSize == 1)
                ...
                throw new KanziIOException("Invalid bitstream, incorrect block checksum size: " + chkSize,
        ...
        try {
            ...
            throw new KanziIOException("Invalid bitstream, unknown entropy codec type: " + this.entropyType,
        ...
        try {
            ...
            throw new KanziIOException("Invalid bitstream, unknown transform type: " + this.transformType,
                    Error.ERR_INVALID_CODEC);
        }
        ...
        if (bsVersion >= 5) {
            ...
            if (cksum1 != (cksum2 & ((1 << crcSize) - 1)))
                throw new KanziIOException("Invalid bitstream, checksum mismatch", Error.ERR_CRC_CHECK);
        } else if (bsVersion >= 3) {
            ...
            if (cksum1 != (cksum2 & 0x0F))
                throw new KanziIOException("Invalid bitstream, corrupted header", Error.ERR_CRC_CHECK);
        } else {
     ...
     * buffer array <code>array</code>. The number of bytes actually read is
     * returned as an integer. This method blocks until input data is available, end
     * of file is detected, or an exception is thrown.
     *
     * <p>
     * If the length of <code>array</code> is zero, then no bytes are read and
     ...
     * read and stored into <code>array</code>.
     *
     * <p>
     * The first byte read is stored into element <code>array[0]</code>, the next
     * one into <code>array[1]</code>, and so on. The number of bytes read is, at
     * most, equal to the length of <code>array</code>. Let <i>k</i> be the number
     ...
     *
     * <p>
     * The <code>read(array)</code> method for class <code>InputStream</code> has
     * the same effect as:
     * 
     * <pre>
     * <code> read(b, 0, array.length) </code>
     * </pre>
     *
     * @param data
     ...
     * @exception IOException
     *                If the first byte cannot be read for any reason other than the
     *                end of the file, if the input stream has been closed, or if
    ...
    public int read(byte[] data, int off, int len) throws IOException {
        ...
        while (remaining > 0) {
            ...
            final int lenChunk = (int) Math.min((long) remaining,
                    Math.min(this.available, (long) (this.bufferThreshold - this.buffers[this.bufferId].index)));

            if (lenChunk > 0) {
                // Process a chunk of in-buffer data. No access to bitstream required
                System.arraycopy(this.buffers[this.bufferId].array, this.buffers[this.bufferId].index, data, off,
                        lenChunk);
    ...
    /**
     * Processes a single block of compressed data, decompressing it into the
     * buffer.
     * 
     * @return The number of bytes successfully processed and decompressed from the
     *         block.
     * @throws IOException
     *             If an I/O error occurs during block processing.
     */
        ...
        try {
            ...
            while (true) {
                ...
                for (int taskId = 0; taskId < nbTasks; taskId++) {
                    ...
                    Callable<Status> task = new DecodingTask(this.buffers[taskId], this.buffers[this.jobs + taskId],
                            blkSize, this.transformType, this.entropyType, firstBlockId + taskId + 1, this.ibs,
                ...
                for (Status res : results) {
                    ...
                    if (blockListeners.length > 0) {
                        ...
                        Event evt = new Event(Event.Type.AFTER_TRANSFORM, res.blockId, res.decoded, res.checksum,
                                hashType, res.completionTime);
    ...
    /**
     * Returns the total number of bytes read so far from the compressed stream. The
     * method calculates the total number of bytes based on the number of bits read
     * by the input bitstream, converting it to bytes.
     * 
    ...
    /**
     * Notifies all listeners of a given event. Each listener's
     * {@link Listener#processEvent(Event)} method is called with the provided
     * event. Any exceptions thrown by the listeners are caught and ignored to
     * prevent failure of the notification process.
     * 
     * @param listeners
     *            An array of listeners to notify.
     * @param evt
     *            The event to pass to each listener.
     */
    ...
    /**
     * A task that decodes a single block of compressed data. This class allows
     * parallel processing of multiple blocks. Each task handles the transformation
     * and entropy decoding of a block independently. The task is executed
     * asynchronously using an {@link ExecutorService}.
     * 
     * <p>
     * The task is constructed with the necessary buffers, configuration, and
     * context for processing the block. The actual decoding logic is executed by
    ...
    static class DecodingTask implements Callable<Status> {
        ...
        /**
         * Constructs a new {@link DecodingTask} for decoding a block of compressed
         * data.
         * 
         * @param iBuffer
         *            The input buffer containing the compressed data.
         * @param oBuffer
         *            The output buffer where the decompressed data will be written.
         * @param blockSize
         *            The size of the block being decoded.
         ...
         *            The input bitstream used to read data for decoding.
         * @param hasher32
         *            The 32-bit hash function used for integrity checks.
         * @param hasher64
         *            The 64-bit hash function used for integrity checks.
         * @param processedBlockId
         *            Atomic integer for tracking the processed block ID.
         * @param listeners
         *            Array of listeners to notify during the decoding process.
         * @param ctx
         *            A map containing the context information for decoding.
         */
        DecodingTask(SliceByteArray iBuffer, SliceByteArray oBuffer, int blockSize, long transformType, int entropyType,
                int blockId, InputBitStream ibs, XXHash32 hasher32, XXHash64 hasher64, AtomicInteger processedBlockId,
         ...
         * input data in the buffer and applies the necessary transformations and
         * entropy decoding.
         *
         * <p>
         * The actual decoding logic is not implemented in this class. The
         * {@link #call()} method should be extended or modified to implement the block
         * decoding functionality.
         * </p>
         *
         * @return The status of the decoding task after execution.
         * @throws Exception
         *             If an error occurs during the execution of the task.
         */
        @Override
        public Status call() throws Exception {
            return this.decodeBlock(this.data, this.buffer, this.transformType, this.entropyType, this.blockId);
        }
        ...
        private Status decodeBlock(SliceByteArray data, SliceByteArray buffer, long blockTransformType,
            ...
            try {
                ...
                final int maxTransformLength = Math.min(Math.max(this.blockSize + this.blockSize / 2, 2048),
                        MAX_BITSTREAM_BLOCK_SIZE);
                ...
                if (this.listeners.length > 0) {
                    ...
                    Event evt1 = new Event(Event.Type.AFTER_ENTROPY, currentBlockId, preTransformLength, checksum1,
                            hashType);

                    notifyListeners(this.listeners, evt1);

                    // Notify before transform (block size after entropy decoding)
                    Event evt2 = new Event(Event.Type.BEFORE_TRANSFORM, currentBlockId, preTransformLength, checksum1,
                            hashType);
                ...
                if (this.hasher32 != null) {
                    ...
                    if (checksum2 != checksum1)
                        return new Status(data, currentBlockId, decoded, checksum1, Error.ERR_CRC_CHECK,
                                "Corrupted bitstream: expected checksum " + Long.toHexString(checksum1) + ", found "
                                        + Long.toHexString(checksum2));
                }
    ...
    /**
     * Represents the status of a decoding task. The status includes information
     * about the decoded block, including the block ID, the number of decoded bytes,
     * the checksum, any errors that occurred, a message, and the completion time.
     *
     * <p>
     * The status is used to track the progress and outcome of the decoding process.
     * </p>
     */
    static class Status {
        final int blockId;
         ...
         * @param blockId
         *            The block ID of the decoded block.
         * @param decoded
         *            The number of bytes decoded.
         * @param checksum
         *            The checksum of the decoded data.
         * @param error
         *            The error code (0 = OK).
         * @param msg
         ...
         *            The message describing the status.
         * @param skipped
         *            Whether the block was skipped during decoding.
         */
