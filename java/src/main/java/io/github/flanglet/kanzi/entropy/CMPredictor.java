...
/**
 * <p>
 * Implementation of a Context Model based predictor. This predictor estimates the probability of
 * the next bit being 1 based on a combination of different contexts and adaptive learning rates.
 * </p>
 *
 * <p>
 * It uses multiple probability counters that are updated based on the actual decoded bit, allowing
 * it to adapt to the characteristics of the input data.
 * </p>
 */
public class CMPredictor implements Predictor {
  ...
  /**
   * The scaling factor for probabilities, representing the maximum possible probability value.
   */
  ...
  private int runMask;
  /**
   * A 2D array of probability counters, used for general context modeling. `counter1[i][j]` stores
   * the probability for context `i` and sub-context `j`.
   */
  private final int[][] counter1;
  /**
   * A 2D array of probability counters, used for more specific context modeling. `counter2[i][j]`
   * stores the probability for context `i` and sub-context `j`.
   */
  private final int[][] counter2;
  /**
   * A flag indicating if the bitstream version is 3 or older, which affects probability
   * calculation.
   */
   ...
   * <p>
   * The predictor is initialized with default probability values and can be configured with a
   * context map to handle different bitstream versions.
   * </p>
   *
   * @param ctx A map containing context information for the predictor, e.g., "bsVersion" to specify
   ...
   * <p>
   * The internal counters are adjusted based on the provided bit and adaptive learning rates. The
   * context is also updated for the next prediction.
   * </p>
   *
   ...
   * <p>
   * The prediction is an integer value in the range [0, 4095], representing the split point in a
   * range coding scheme.
   * </p>
   *
