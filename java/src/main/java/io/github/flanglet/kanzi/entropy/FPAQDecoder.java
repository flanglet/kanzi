...
/**
 * <p>
 * Implementation of an FPAQ decoder. This class is derived from fpaq0r by Matt
...
public class FPAQDecoder implements EntropyDecoder {
    ...
    /**
     * A 2D array storing probability models for different contexts. `probs[i][j]`
     * represents the probability of the next bit being 1 in context `j` for a
     * specific context level `i`.
     */
     ...
     * the current value. The probability model for the current context is then
     * updated based on the decoded bit.
     * </p>
     *
     * @param pred
