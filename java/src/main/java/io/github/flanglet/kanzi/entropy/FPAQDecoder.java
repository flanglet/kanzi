...
/**
 * <p>
 * Implementation of an FPAQ decoder. This class is derived from fpaq0r by Matt Mahoney and
 * Alexander Ratushnyak, and is a simple (and fast) adaptive entropy bit coder.
 * </p>
 *
 * <p>
...
public class FPAQDecoder implements EntropyDecoder {
  ...
  public int decode(byte[] block, int blkptr, int count) {
    ...
    while (startChunk < end) {
      ...
      if ((szBytes < 0) || ((long) szBytes > ((long) chunkSize << 1)))
        return 0;

      if (this.sba.array.length < szBytes)
        this.sba.array = new byte[szBytes];
   ...
   * The probability model for the current context is then updated based on the decoded bit.
   * </p>
   *
   * @param pred The prediction value (probability) for the bit, typically in the range [0, PSCALE].
