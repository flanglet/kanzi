...
/**
 * <p>
 * Implementation of a static Huffman encoder. This class encodes symbols into a bitstream using
 * Huffman codes, which are variable-length codes optimized for symbol frequencies.
 * </p>
 *
 * <p>
 * The encoder uses an in-place generation of canonical codes rather than a traditional Huffman
 * tree, which can be more memory-efficient. It supports chunk-based encoding, where frequency
 * statistics are recomputed for each chunk of data.
 * </p>
 */
public class HuffmanEncoder implements EntropyEncoder {
  ...
  /**
   * Creates a new {@code HuffmanEncoder} with the specified output bitstream and a default chunk
   * size.
   *
  ...
  /**
   * Creates a new {@code HuffmanEncoder} with the specified output bitstream and chunk size.
   *
   * @param bitstream The {@link OutputBitStream} to write compressed data to.
  ...
  /**
   * Updates the frequencies of symbols and rebuilds the Huffman codes based on the new frequencies.
   *
   * @param freqs An array containing the frequencies of symbols.
   * @return The number of symbols in the alphabet.
   * @throws BitStreamException if an error occurs during bitstream operations or if code generation
   *         fails.
   */
  private int updateFrequencies(int[] freqs) throws BitStreamException {
    ...
    if (count == 1) {
      ...
      if (maxCodeLen > HuffmanCommon.MAX_SYMBOL_SIZE_V4) {
        ...
        HuffmanCommon.generateCanonicalCodes(sizes, this.codes, ranks, count,
  ...
  /**
   * Limits the code lengths to {@code HuffmanCommon.MAX_SYMBOL_SIZE_V4} by adjusting the code
   * lengths and repaying any bit debt.
   *
   * @param alphabet The array of symbols in the alphabet.
   * @param freqs The frequencies of the symbols.
   * @param sizes The array to store the adjusted code lengths.
   * @param ranks The array of symbols sorted by frequency.
   ...
   *
   * @param sizes The array to store the computed code lengths.
   * @param ranks The array of symbols sorted by frequency (will be modified).
   * @param count The number of symbols in the alphabet.
   * @return The maximum code length, or 0 if an error occurs (e.g., zero frequency).
  ...
  /**
   * First phase of the in-place calculation of minimum-redundancy codes. This method computes the
   * sums of frequencies for internal nodes.
   *
   * @param data An array containing frequencies (will be modified).
  ...
  /**
   * Second phase of the in-place calculation of minimum-redundancy codes. This method assigns code
   * lengths to symbols based on the results of phase 1.
   *
   * @param data An array containing intermediate results from phase 1 (will be modified).
   * @param n The number of symbols (must be at least 2).
