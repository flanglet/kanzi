...
/**
 * A comparator for comparing elements in an integer array. This class implements the
 * {@link ArrayComparator} interface and provides a mechanism to compare two elements based on their
 * values. The comparison also accounts for stable sorting by considering their indices when the
 * values are equal.
 *
 * <p>
 * This class is immutable and thread-safe as it holds a reference to the input array but does not
 * modify it.
 * </p>
 *
 * <p>
...
public final class DefaultArrayComparator implements ArrayComparator {
   ...
   * <p>
   * The comparison is based on the values of the elements at the provided indices. If the values
   * are equal, the method returns a comparison based on their indices to maintain stability in
   * sorting.
   * </p>
   *
   * @param lidx the index of the first element to compare
   * @param ridx the index of the second element to compare
   * @return a negative integer if the element at {@code lidx} is less than the element at
   *         {@code ridx}, a positive integer if the element at {@code lidx} is greater than the
   *         element at {@code ridx}, or zero if they are equal
   */
  @Override
