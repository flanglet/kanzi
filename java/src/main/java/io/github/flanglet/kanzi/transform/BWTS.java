...
/**
 * Bijective version of the Burrows-Wheeler Transform. The main advantage over the regular BWT is
 * that there is no need for a primary index (hence the bijectivity). BWTS is about 10% slower than
 * BWT. Forward transform based on the code at https://code.google.com/p/mk-bwts/ by Neal Burns and
 * DivSufSort (port of libDivSufSort by Yuta Mori).
 */
public class BWTS implements ByteTransform {
   ...
   * @return the new rank
   */
  private static int moveLyndonWordHead(int[] sa, int[] isa, byte[] data, int count, int srcIdx,
      int start, int size, int rank) {
    ...
    while (rank + 1 < count) {
      ...
      while ((k < size) && (nextStart < count)
          && (data[startIdx + k] == data[srcIdx + nextStart])) {
        k++;
        nextStart++;
      }
      ...
      if ((k < size) && (nextStart < count)
          && ((data[startIdx + k] & 0xFF) < (data[srcIdx + nextStart] & 0xFF)))
        break;
