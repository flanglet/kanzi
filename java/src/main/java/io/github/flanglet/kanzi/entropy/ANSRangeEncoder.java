 ...
 * been ported from https://github.com/rygorous/ryg_rans
 */
public class ANSRangeEncoder implements EntropyEncoder {
  ...
  /**
   * The default log range for the ANS encoder. This value determines the precision of the frequency
   * table.
   */
  ...
  /**
   * Creates a new ANSRangeEncoder with a default order of 0, default chunk size, and default log
   * range.
   *
  ...
  /**
   * Creates a new ANSRangeEncoder with the specified order, default chunk size, and default log
   * range.
   *
   * @param bs The OutputBitStream to write compressed data to.
  ...
  /**
   * Creates a new ANSRangeEncoder with the specified order, chunk size, and default log range.
   *
   * @param bs The OutputBitStream to write compressed data to.
   * @param order The order of the ANS model (0 or 1).
  ...
  /**
   * Creates a new ANSRangeEncoder with the specified order, chunk size, and log range.
   *
   * @param bs The OutputBitStream to write compressed data to.
   * @param order The order of the ANS model (0 or 1).
   * @param chunkSize The size of data chunks to process.
   * @param logRange The log range for the ANS model. This value determines the precision of the
  ...
  public ANSRangeEncoder(OutputBitStream bs, Map<String, Object> ctx, int order) {
    ...
    this.chunkSize =
        (int) Math.min(((long) DEFAULT_ANS0_CHUNK_SIZE) << (8 * order), MAX_CHUNK_SIZE);
   ...
   *
   * @param frequencies A 2D array containing the frequencies for each context and symbol.
  ...
  /**
   * Rebuilds the statistics (frequencies) for a chunk of data and updates the ANS header.
   *
  ...
  private int rebuildStatistics(byte[] block, int start, int end, int lr) {
    ...
    if (this.order == 0) {
      ...
      if (quarter == 0) {
        ...
        Global.computeHistogramOrder1(block, start + 0 * quarter, start + 1 * quarter, this.freqs,
            true);
        Global.computeHistogramOrder1(block, start + 1 * quarter, start + 2 * quarter, this.freqs,
            true);
        Global.computeHistogramOrder1(block, start + 2 * quarter, start + 3 * quarter, this.freqs,
            true);
        Global.computeHistogramOrder1(block, start + 3 * quarter, start + 4 * quarter, this.freqs,
            true);
      }
    }
