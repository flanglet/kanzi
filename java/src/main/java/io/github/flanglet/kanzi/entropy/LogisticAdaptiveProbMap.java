...
/**
 * <p>
 * Implementation of an Adaptive Probability Map (APM) with logistic function. This class maps a
 * probability and a context into a new probability that the next bit will be 1. After each guess,
 * it updates its state to improve future guesses.
 * </p>
 *
 * <p>
 * It uses a logistic function to squash the prediction and adapts its internal probabilities based
 * on the actual bit observed and a learning rate.
 * </p>
 */
/* package */ final class LogisticAdaptiveProbMap {
  /**
   * The index into the {@code data} array, representing the last probability and context.
   */
  private int index;

  /**
   * The update rate for adapting probabilities. A smaller rate means faster adaptation.
   */
  private final int rate;

  /**
   * The internal data array storing probabilities for different contexts. Each entry is a packed
   * integer representing a probability.
   */
  private final int[] data;

  /**
   * Creates a new {@code LogisticAdaptiveProbMap}.
   *
  ...
  /**
   * Returns an improved prediction given the current bit, prediction, and context.
   *
