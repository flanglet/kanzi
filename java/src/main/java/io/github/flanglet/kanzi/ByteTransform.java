...
public interface ByteTransform {
     ...
     * array in the forward direction. Read src.length bytes from
     * src.array[src.index], process them and write them to dst.array[dst.index].
     * The index of each slice is updated with the number of bytes respectively read
     * from and written to.
     *
     * @param src
     *            the source {@code SliceByteArray} containing the data to be
     *            processed
     * @param dst
     *            the destination {@code SliceByteArray} where the processed data
     *            will be written
    ...
    /**
     * Processes the source array and writes the transformed data to the destination
     * array in the inverse direction. Read src.length bytes from
     * src.array[src.index], process them and write them to dst.array[dst.index].
     * The index of each slice is updated with the number of bytes respectively read
     * from and written to.
     *
