...
public class SA_IS {
  ...
  private static void sortLMSSuffixes(SliceIntArray src, int[] sa, SliceIntArray C, SliceIntArray B,
      int n, int k) {
    // compute sal
    if (C == B)
      getCounts(src, C, n, k);
  ...
  public static int computeSuffixArray(SliceIntArray data, int[] sa, int fs, int n, int k,
      boolean isBWT) {
    SliceIntArray C;
    SliceIntArray B;
    int flags;
