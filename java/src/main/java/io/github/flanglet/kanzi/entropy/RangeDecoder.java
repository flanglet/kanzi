 ...
 * original symbols based on their probabilities. It maintains a current range and a code
 * value, narrowing the range with each decoded symbol and normalizing it by reading
 * more bits from the bitstream when necessary.
 * </p>
 *
 * <p>
...
public final class RangeDecoder implements EntropyDecoder {
   ...
   /**
    * Creates a new {@code RangeDecoder} with the specified input bitstream
   ...
   /**
    * Creates a new {@code RangeDecoder} with the specified input bitstream
    * and chunk size. The chunk size indicates how many bytes are decoded
    * (per block) before resetting the frequency statistics.
    *
    * @param bitstream The {@link InputBitStream} to read compressed data from.
   ...
   /**
    * Decodes the header of a data chunk, which includes the alphabet,
    * frequencies, and log range. This information is used to set up the
    * decoding tables for the current chunk.
    *
    ...
    *                            incorrect Huffman symbol, frequency size, or
    *                            frequency value, or if the
    ...
    * This method uses the current range, code, and frequency tables to
