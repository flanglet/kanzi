...
/**
 * <p>
 * Implementation of a range decoder based on the algorithm described by G.N.N. Martin in his
 * seminal article in 1979, and optimized for speed. This implementation is derived from Dmitry
 * Subbotin's Order 0 range coder.
 * </p>
 *
 * <p>
 * A range decoder reads a compressed stream of data and reconstructs the original symbols based on
 * their probabilities. It maintains a current range and a code value, narrowing the range with each
 * decoded symbol and normalizing it by reading more bits from the bitstream when necessary.
 * </p>
 *
 * <p>
...
public final class RangeDecoder implements EntropyDecoder {
  ...
  /**
   * The current code value read from the bitstream, representing a point within the range.
   */
  private long code;
  /**
  ...
  /**
   * The current shift value, derived from the log range, used in range calculations.
   */
  private int shift;

  /**
   * Creates a new {@code RangeDecoder} with the specified input bitstream and a default chunk size.
   *
  ...
  /**
   * Creates a new {@code RangeDecoder} with the specified input bitstream and chunk size. The chunk
   * size indicates how many bytes are decoded (per block) before resetting the frequency
   * statistics.
   *
   * @param bitstream The {@link InputBitStream} to read compressed data from.
  ...
  /**
   * Decodes the header of a data chunk, which includes the alphabet, frequencies, and log range.
   * This information is used to set up the decoding tables for the current chunk.
   *
   * @param frequencies An array to store the decoded frequencies of symbols.
   ...
   *         symbol, frequency size, or frequency value, or if the bitstream is corrupted.
   */
  protected int decodeHeader(int[] frequencies) {
      ...
      for (int j = i; j < endj; j++) {
        ...
        if ((freq <= 0) || (freq >= scale)) {
          throw new BitStreamException("Invalid bitstream: incorrect frequency " + freq
              + " for symbol '" + this.alphabet[j] + "' in range decoder",
              BitStreamException.INVALID_STREAM);
        }
    ...
    if (scale <= sum) {
      throw new BitStreamException(
          "Invalid bitstream: incorrect frequency " + frequencies[this.alphabet[0]]
              + " for symbol '" + this.alphabet[0] + "' in range decoder",
          BitStreamException.INVALID_STREAM);
    }
   ...
   * byte array. It processes data in chunks, resetting frequency statistics for each chunk.
   * </p>
   *
  ...
  /**
   * Decodes a single byte from the bitstream. This method uses the current range, code, and
   * frequency tables to determine the next symbol.
   *
