...
/**
 * <p>
 * Implementation of a range encoder based on the algorithm described by G.N.N. Martin in his
 * seminal article in 1979, and optimized for speed. This implementation is derived from Dmitry
 * Subbotin's Order 0 range coder.
 * </p>
 *
 * <p>
 * A range encoder writes a compressed stream of data by maintaining a current range and a low
 * value. For each symbol, it narrows the range based on the symbol's probability and normalizes the
 * range by writing bits to the bitstream when necessary.
 * </p>
 *
 * <p>
...
public final class RangeEncoder implements EntropyEncoder {
  ...
  /**
   * The bottom value for the range, used in range coding. When the range falls below this value,
   * normalization occurs.
   */
  private static final long BOTTOM_RANGE = 0x000000000000FFFFL;
  /**
   * A mask used to check if the most significant bits of the low and (low + range) values are the
   * same, indicating that bits can be shifted out.
   */
  private static final long RANGE_MASK = 0x0FFFFFFF00000000L;
  /**
   * The default chunk size for processing data. This value determines how many bytes are processed
   * before frequency statistics are reset.
   */
  private static final int DEFAULT_CHUNK_SIZE = 1 << 15; // 32 KB by default
  /**
   * The default log range for the range encoder. This value determines the precision of the
   * frequency table.
   */
  ...
  /**
   * The cumulative frequencies of symbols, used to determine the sub-range for each symbol.
   */
  ...
  /**
   * Creates a new {@code RangeEncoder} with the specified output bitstream and a default chunk size
   * and log range.
   *
  ...
  /**
   * Creates a new {@code RangeEncoder} with the specified output bitstream, chunk size, and log
   * range. The chunk size indicates how many bytes are encoded (per block) before resetting the
   * frequency statistics.
   *
   * @param bs The {@link OutputBitStream} to write compressed data to.
   * @param chunkSize The size of data chunks to process. Must be at least 1024 and at most
   *        {@link #MAX_CHUNK_SIZE}.
   * @param logRange The log range for the range encoder. This value determines the precision of the
   *        frequency table. Must be in the range [8, 16].
   * @throws NullPointerException if {@code bs} is {@code null}.
   * @throws IllegalArgumentException if {@code chunkSize} or {@code logRange} is out of range.
   */
  ...
  /**
   * Updates the frequencies and cumulative frequencies for the current chunk, and encodes the
   * header into the bitstream.
   *
  ...
  /**
   * Encodes the header of a data chunk, which includes the alphabet, frequencies, and log range.
   *
   ...
   * @return {@code true} if the header was encoded successfully, {@code false} otherwise.
   */
   ...
   * <p>
   * This method reads data from the provided byte array, encodes it using the range coding
   * algorithm, and writes the compressed data to the internal bitstream. It processes data in
   * chunks, resetting frequency statistics for each chunk.
   * </p>
   *
  ...
  /**
   * Computes the frequencies for a chunk of data, updates the cumulative frequencies, and encodes
   * the chunk header.
   *
  ...
  /**
   * Disposes of any resources used by the encoder. This method currently does nothing as there are
