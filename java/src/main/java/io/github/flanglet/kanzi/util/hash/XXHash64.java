...
/**
 * XXHash64 is an implementation of the 64-bit variant of the XXHash algorithm, which is a fast
 * non-cryptographic hash function. It is designed for high-speed hashing, and is widely used for
 * checksums and hashing large amounts of data. This class allows for a configurable seed value, and
 * provides methods for hashing byte arrays of various lengths. Port to Java of the original source
 * code: https://github.com/Cyan4973/xxHash
 * 
 * <p>
 * The algorithm processes the input data in blocks and uses a combination of mix functions and
 * bitwise operations to produce a hash value. It is optimized for 64-bit platforms and can be used
 * for general-purpose hashing where cryptographic security is not a concern.
 * 
 */
public class XXHash64 {
  ...
  /**
   * Default constructor that initializes the hash function with a seed based on the current system
   * time in nanoseconds.
   */
  public XXHash64() {
  ...
  /**
   * Sets the seed value for the hash computation. This allows for custom seed values to modify the
   * output hash.
   * 
  ...
  /**
   * Computes the 64-bit hash of the provided byte array. This method uses the entire byte array,
  ...
  /**
   * Computes the 64-bit hash of the provided byte array, with the option to specify an offset and
