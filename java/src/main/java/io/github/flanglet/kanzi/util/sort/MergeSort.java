...
/**
 * The {@code MergeSort} class implements the merge sort algorithm, which is a divide-and-conquer
 * comparison-based sorting algorithm. Merge sort divides the input array into smaller sub-arrays,
 * recursively sorts each sub-array, and then merges the sorted sub-arrays back together. While
 * conceptually simple, it is usually not very performant for smaller arrays due to its recursive
 * nature. However, merge sort is known for its stable sorting and predictable O(n log n) time
 * complexity.
 *
 * <p>
 * Merge sort is efficient for large datasets and nearly sorted data, but it can require significant
 * memory overhead due to the need for auxiliary space to store the merged sub-arrays. This
 * implementation uses insertion sort for small sub-arrays to improve performance on small or nearly
 * sorted datasets.
 * </p>
 *
 * <p>
...
public class MergeSort implements IntSorter {
  ...
  /**
   * Constructs a new {@code MergeSort} instance. This constructor initializes an empty buffer for
   * merging and uses an {@code InsertionSort} instance for sorting small arrays.
   */
  public MergeSort() {
   ...
   *
   * <p>
   * This method divides the array into smaller sub-arrays, recursively sorts them using merge sort,
   * and then merges the sorted sub-arrays back together. For small sub-arrays (less than
   * {@code SMALL_ARRAY_THRESHOLD}), insertion sort is used for efficiency.
   * </p>
   *
   ...
   *         provided (out-of-bounds indices).
   */
  @Override
   ...
   *
   * <p>
   * This method splits the array into two halves and recursively sorts each half. Once the
   * sub-arrays are sorted, they are merged together using the {@code merge} method.
   * </p>
   *
   * @param data the array to be sorted.
   ...
   *
   * <p>
   * This method performs the merging step of merge sort. It copies the sorted elements from the
   * left and right halves of the sub-array into a temporary buffer and then merges them back into
   * the original array.
   * </p>
   *
