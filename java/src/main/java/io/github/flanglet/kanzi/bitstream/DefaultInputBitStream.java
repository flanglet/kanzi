...
/**
 * A default implementation of the {@link InputBitStream} interface that reads
 * bits from an input stream. This class buffers input data for efficient
 * bit-level reading and handles various edge cases, including end-of-stream
 * conditions.
 * <p>
 * The input stream is read into a byte buffer, from which bits are extracted as
 * needed. The buffer size must be a multiple of 8 and at least 1024 bytes.
 * </p>
 */
public final class DefaultInputBitStream implements InputBitStream {
    ...
    /**
     * Constructs a DefaultInputBitStream with the specified input stream and buffer
     * size.
     *
     * @param is
     *            the InputStream to read bits from
     * @param bufferSize
     ...
     *             if the stream is closed or an error occurs
     */
    @Override
     ...
     *
     * @param count
     *            the number of bits to read (must be in the range [1..64])
     ...
     *
     * @param bits
     *            the array to store the read bits
     * @param start
    ...
    public int readBits(byte[] bits, int start, int count) throws BitStreamException {
        ...
        if ((count < 0) || ((count >> 3) > bits.length - start))
            throw new IllegalArgumentException(
                    "Invalid bit count: " + count + " (must be in [1.." + (((long) (bits.length - start)) << 3) + "])");
    ...
    /**
     * Reads a specified number of bytes from the underlying input stream into the
     * buffer. This method is responsible for handling stream closure and ensuring
     * that data is available for subsequent bit reads.
     *
     * @param count
     *            the number of bytes to read from the input stream
     * @return the number of bytes actually read, or -1 if the end of the stream is
     *         reached
     * @throws BitStreamException
     *             if the stream is closed or no more data can be read
     */
