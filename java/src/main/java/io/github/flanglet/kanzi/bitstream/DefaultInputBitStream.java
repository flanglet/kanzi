...
/**
 * A default implementation of the {@link InputBitStream} interface that reads bits from an input
 * stream. This class buffers input data for efficient bit-level reading and handles various edge
 * cases, including end-of-stream conditions.
 * <p>
 * The input stream is read into a byte buffer, from which bits are extracted as needed. The buffer
 * size must be a multiple of 8 and at least 1024 bytes.
 * </p>
 */
public final class DefaultInputBitStream implements InputBitStream {
  ...
  /**
   * Constructs a DefaultInputBitStream with the specified input stream and buffer size.
   *
  ...
  /**
   * Reads a specified number of bytes from the underlying input stream into the buffer. This method
   * is responsible for handling stream closure and ensuring that data is available for subsequent
   * bit reads.
   *
