...
/**
 * The Burrows-Wheeler Transform (BWT) is a reversible transform based on permutation of the data in
 * the original message to reduce the entropy.
 * <p>
 * The initial text can be found here: Burrows M and Wheeler D, [A block sorting lossless data
 * compression algorithm] Technical Report 124, Digital Equipment Corporation, 1994
 * <p>
 * See also Peter Fenwick, [Block sorting text compression - final report] Technical Report 130,
 * 1996
 * <p>
 * This implementation replaces the 'slow' sorting of permutation strings with the construction of a
 * suffix array (faster but more complex). The suffix array contains the indexes of the sorted
 * suffixes.
 * <p>
 * E.G. 0123456789A Source: mississippi\0 Suffixes: rank sorted mississippi\0 0 -&gt; 4 i\0
 * ississippi\0 1 -&gt; 3 ippi\0 ssissippi\0 2 -&gt; 10 issippi\0 sissippi\0 3 -&gt; 8 ississippi\0
 * issippi\0 4 -&gt; 2 mississippi\0 ssippi\0 5 -&gt; 9 pi\0 sippi\0 6 -&gt; 7 ppi\0 ippi\0 7 -&gt;
 * 1 sippi\0 ppi\0 8 -&gt; 6 sissippi\0 pi\0 9 -&gt; 5 ssippi\0 i\0 10 -&gt; 0 ssissippi\0 Suffix
 * array SA: 10 7 4 1 0 9 8 6 3 5 2 BWT[i] = input[SA[i]-1] =&gt; BWT(input) = ipssmpissii (+
 * primary index 5) The suffix array and permutation vector are equal when the input is 0 terminated
 * The insertion of a guard is done internally and is entirely transparent.
 * <p>
 * This implementation extends the canonical algorithm to use up to MAX_CHUNKS primary indexes
 * (based on input block size). Each primary index corresponds to a data chunk. Chunks may be
 * inverted concurrently.
 */
public class BWT implements ByteTransform {
   ...
   * @throws IllegalArgumentException if the number of jobs is not positive or the thread pool is
   *         null when the number of jobs is greater than 1
   */
  ...
  private boolean inverseMergeTPSI(SliceByteArray src, SliceByteArray dst, int count) {
    ...
    if (getBWTChunks(count) != 8) {
      ...
      if ((t0 >= count) || (t1 >= count) || (t2 >= count) || (t3 >= count) || (t4 >= count)
          || (t5 >= count) || (t6 >= count) || (t7 >= count))
        return false;
  ...
  class InverseBiPSIv2Task implements Callable<Integer> {
     ...
     */
    public InverseBiPSIv2Task(byte[] output, int dstIdx, int total, int ckSize, int firstChunk,
        int lastChunk) {
      this.output = output;
      this.dstIdx = dstIdx;
      this.ckSize = ckSize;
