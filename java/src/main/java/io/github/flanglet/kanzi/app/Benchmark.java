 ...
 *
 * A comprehensive benchmarking tool for the Kanzi Java compression library that tests various
 * transform and entropy coding combinations with timing and compression ratio analysis.
 *
...
public class Benchmark {
  ...
  private static final String[] TRANSFORMS = {"NONE", "PACK", "BWT", "BWTS", "LZ", "LZX", "LZP",
      "ROLZ", "ROLZX", "RLT", "ZRLT", "MTFT", "RANK", "SRT", "TEXT", "EXE", "MM", "UTF", "DNA"};

  private static final String[] OPTIMIZED_TRANSFORMS = {"TEXT", "RLT", "PACK", "ZRLT", "BWTS",
      "BWT", "LZP", "MTFT", "SRT", "LZ", "LZX", "ROLZ", "ROLZX", "RANK", "EXE", "MM"};

  private static final String[] ENTROPY_CODERS =
      {"NONE", "HUFFMAN", "ANS0", "ANS1", "RANGE", "CM", "FPAQ", "TPAQ", "TPAQX"};

  private static final String[] SPECIALIZED_TRANSFORMS = {"RLT", "PACK", "PACK+ZRLT+PACK",
      "PACK+RLT", "RLT+PACK", "RLT+TEXT+PACK", "RLT+PACK+LZP", "RLT+PACK+LZP+RLT", "TEXT+ZRLT+PACK",
      "RLT+LZP+PACK+RLT", "TEXT+ZRLT+PACK+LZP", "TEXT+RLT+PACK", "TEXT+RLT+LZP",
      "TEXT+RLT+PACK+LZP", "TEXT+RLT+LZP+RLT", "TEXT+RLT+PACK+LZP+RLT", "TEXT+RLT+LZP+PACK",
      "TEXT+RLT+PACK+RLT+LZP", "TEXT+RLT+LZP+PACK+RLT", "TEXT+PACK+RLT", "EXE+TEXT+RLT+UTF+PACK",
      "EXE+TEXT+RLT+UTF+DNA", "EXE+TEXT+RLT", "EXE+TEXT", "TEXT+BWTS+SRT+ZRLT", "BWTS+SRT+ZRLT",
      "TEXT+BWTS+MTFT+RLT", "BWTS+MTFT+RLT", "TEXT+BWT+MTFT+RLT", "BWT+MTFT+RLT"};
  ...
  private void printHeader() {
    ...
    System.out.println();
    System.out.printf("%12s %10s %9s %10s %s%n", "COMPRESSED", "TIME", "RATIO", "SPEED",
        "ALGORITHM");
  ...
  private void runLargeBlockSizes() throws Exception {
    ...
    for (String blockSize : blockSizes) {
      benchmarkKanzi("kanzi -b" + blockSize + " -l9",
          new String[] {"--block=" + blockSize, "--level=9"});
    }
  }
  ...
  private void analyzeResults() {
    ...
    CompressionResult bestCompression =
        results.stream().min(Comparator.comparing(r -> r.ratio)).orElse(null);
  ...
  static class CompressionResult {
    ...
    CompressionResult(String algorithm, long originalSize, long compressedSize,
        double timeSeconds) {
      ...
      this.speedMBps = timeSeconds > 0 ? (originalSize / (1024.0 * 1024.0)) / timeSeconds
