...
/**
 * The {@code Kanzi} class is a command-line application for compressing and
 * decompressing data using a fast lossless algorithm.
 *
 * <p>
 * It provides functionalities for processing command line arguments and
 * executing compression and decompression based on those arguments.
 * </p>
 */
public class Kanzi {
    /**
     ...
     */
    private static final String[] CMD_LINE_ARGS = new String[]{"-c", "-d", "-i", "-o", "-b", "-t", "-e", "-j", "-v",
            "-l", "-s", "-x", "-f", "-h"};
    ...
    /**
     * Processes the command line arguments and populates the provided map with
     * options.
     *
     * @param args
     *            the command line arguments
     * @param map
     *            a map to store processed options and their values
    ...
    private static int processCommandLine(String[] args, Map<String, Object> map) {
        ...
        for (String arg : args) {
            ...
            if (arg.startsWith("--entropy=") || (ctx == ARG_IDX_ENTROPY)) {
                String name = arg.startsWith("--entropy=") ? arg.substring(10).trim().toUpperCase() : arg.toUpperCase();
            ...
            if (arg.startsWith("--transform=") || (ctx == ARG_IDX_TRANSFORM)) {
                String name = arg.startsWith("--transform=")
                        ? arg.substring(12).trim().toUpperCase()
                        : arg.toUpperCase();
            ...
            if (arg.startsWith("--level=") || (ctx == ARG_IDX_LEVEL)) {
                String name = arg.startsWith("--level=") ? arg.substring(8).trim().toUpperCase() : arg.toUpperCase();
            ...
            if (arg.startsWith("--checksum=")) {
                String name = arg.startsWith("--checksum=")
                        ? arg.substring(11).trim().toUpperCase()
                        : arg.toUpperCase();
            ...
            if (arg.startsWith("--block=") || (ctx == ARG_IDX_BLOCK)) {
                String name = arg.startsWith("--block=") ? arg.substring(8).toUpperCase().trim() : arg.toUpperCase();
     ...
     *
     * @param mode
     *            the mode of operation (compress or decompress)
     * @param showHeader
     *            whether to show the application header
     */
     ...
     *
     * @param val
     *            the value associated with the warning
     * @param reason
     *            the reason for the warning
     * @param verbose
     *            verbosity level controlling the output
     */
