...
/**
 * The {@code Kanzi} class is a command-line application for compressing and decompressing data
 * using a fast lossless algorithm.
 *
 * <p>
 * It provides functionalities for processing command line arguments and executing compression and
 * decompression based on those arguments.
 * </p>
 */
public class Kanzi {
  /**
   ...
   */
  private static final String[] CMD_LINE_ARGS = new String[] {"-c", "-d", "-i", "-o", "-b", "-t",
      "-e", "-j", "-v", "-l", "-s", "-x", "-f", "-h", "-y"};
  ...
  private static final String KANZI_VERSION = "2.5.0";
  ...
  private static final String APP_USAGE =
      "Usage: java -jar kanzi.jar [-c|-d|-y] [flags and files in any order]";
  ...
  private static int processCommandLine(String[] args, Map<String, Object> map) {
    ...
    for (String arg : args) {
      ...
      if (arg.equals("--compress") || (arg.equals("-c"))) {
        if (mode != ' ') {
          System.err.println("Only one mode can be provided (already got '" + mode + "'" );
      ...
      if (arg.equals("--info") || (arg.equals("-y"))) {
        if (mode != ' ') {
          System.err.println("Only one mode can be provided (already got '" + mode + "'" );
          return io.github.flanglet.kanzi.Error.ERR_INVALID_PARAM;
        }
    ...
    if (outputName.length() == 0) {
      if (inputName.length() == 0)
        ...
        if (mode == 'y') {
          System.err.println("Warning: ignoring option [" + outputName + "]. Only applicable in compression or decompression mode.");
        }
    ...
    for (String arg : args) {
      ...
      if (arg.equals("--compress") || arg.equals("-c") || arg.equals("--decompress")
          || arg.equals("-d") || (arg.equals("--info") || arg.equals("-y"))) {
        if (ctx != -1) {
      ...
      if (arg.equals("--force") || arg.equals("-f")) {
        if (ctx != -1) {
          ...
          printWarning(arg, " Duplicate 'overwrite' option.", verbose);
        }
      ...
      if (arg.equals("--rm")) {
        if (ctx != -1) {
          ...
          printWarning(arg, " Duplicate 'remove' option.", verbose);
        }
      ...
      if (arg.equals("--skip-dot-files")) {
        if (ctx != -1) {
          ...
          printWarning(arg, " Duplicate 'skip-dot-files' option.", verbose);
        }
      ...
      if (arg.startsWith("--level=") || (ctx == ARG_IDX_LEVEL)) {
        String name =
            arg.startsWith("--level=") ? arg.substring(8).trim().toUpperCase() : arg.toUpperCase();
      ...
      if (arg.startsWith("--block=") || (ctx == ARG_IDX_BLOCK)) {
        String name =
            arg.startsWith("--block=") ? arg.substring(8).toUpperCase().trim() : arg.toUpperCase();
  ...
  private static void printHelp(char mode, int verbose, boolean showHeader) {
    ...
    if ((mode != 'c') && (mode != 'd') && (mode != 'y')) {
      ...
      printOut("   -y, --info", true);
      printOut("        Print information about the input file or directory.\n", true);
    ...
    printOut("        Provide " + File.separator
        + ". at the end of the directory name to avoid recursion", true);
    ...
    if (mode != 'y') {
      ...
      if (mode == 'c') {
        ...
        printOut("        <inputName.knz> if input is <inputName> or 'stdout' if input is 'stdin').",
            true);
        printOut("        or 'none' or 'stdout'.\n", true);
      } else if (mode == 'd') {
        printOut("        Optional name of the output file or directory (defaults to", true);
        printOut("        <inputName> if input is <inputName.knz> or 'stdout' if input is 'stdin').",
            true);
        printOut("        or 'none' or 'stdout'.\n", true);
      } else {
    ...
    if (mode == 'c') {
      ...
      printOut(
          "        Size of blocks (default 4|8|16|32 MiB based on level, max 1 GiB, min 1 KiB).",
          true);
    ...
    if (mode == 'c') {
      ...
      printOut("       This implementation uses a linear time forward transform and parallel",
          true);
      ...
      printOut("  LZP: Lempel Ziv Prediction can be described as an LZ implementation with only",
          true);
      printOut("       one possible match (no offset is emitted).\n", true);
      printOut("  RLT: Run Length Transform is a simple transform that replaces runs of similar",
          true);
      printOut("       symbols with a compact representation.\n", true);
      printOut("  ZRLT: Zero Run Length Transform. Similar to RLT but only processes runs of 0.",
          true);
      printOut("        Usually used post BWT.\n", true);
      printOut("  MTFT: Move-To-Front Transform is a transform that reduces entropy by assigning",
          true);
      printOut(
          "        shorter symbols to recent data (like a LRU cache). Usually used post BWT.\n",
          true);
      printOut(
          "  RANK: Rank Transform is a transform that that reduces entropy by assigning shorter",
          true);
      printOut("        symbols based on symbol frequency ranks. Usually used post BWT.\n", true);
      printOut("  EXE: a transform that reduces the entropy of executable files (X86 & ARM64)",
          true);
      printOut("       by replacing relative jump addresses with absolute ones.\n", true);
      printOut("  TEXT: a text transform that uses a dictionary to replace common words with",
          true);
      ...
      printOut("  ROLZX: Extended ROLZ with more match searches and a more compact encoding.\n",
          true);
      printOut("  SRT: Sorted Rank Transform is a transform that that reduces entropy by assigning",
          true);
      ...
      printOut("  Huffman: a fast implementation of canonical Huffman. Both encoder and decoder",
          true);
      ...
      printOut("  ANS: based on Range Asymmetric Numeral Systems by Jarek Duda (specifically",
          true);
      printOut(
          "       an implementation by Fabian Giesen). Works in a similar fashion to the Range",
          true);
      printOut(
          "       codec but uses only 1 state instead of 2, and encodes in reverse byte order.\n",
          true);
      printOut("  FPAQ: a binary arithmetic codec based on FPAQ1 by Matt Mahoney. Uses a simple",
          true);
      printOut("        adaptive order 0 predictor based on frequencies.\n", true);
      printOut("  CM: a binary arithmetic codec derived from BCM by Ilya Muravyov. Uses context",
          true);
      printOut("      mixing of counters to generate a prediction of the next bit value.\n", true);
      printOut("  TPAQ: a binary arithmetic codec based initially on Tangelo 2.4 (itself derived",
          true);
      printOut("        from FPAQ8). Uses context mixing of predictions produced by one layer",
          true);
      ...
      printOut(
          ...
          true);
      printOut("  TPAQX: Extended TPAQ with more predictions and more memory usage. Slowest but",
          true);
      ...
      printOut(
          "                      --transform=BWT+MTFT+ZRLT --block=4m --entropy=FPAQ --jobs=4\n",
          true);
    }
