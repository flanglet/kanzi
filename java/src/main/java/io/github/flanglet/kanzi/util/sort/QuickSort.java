...
/**
 * Implementation of the Dual-Pivot Quicksort algorithm by Vladimir Yaroslavskiy, Jon Bentley, and
 * Josh Bloch. This algorithm is a variation of the classic Quicksort that uses two pivots instead
 * of one, providing improved performance on certain types of data.
 *
 ...
 *
 * <p>
 * The algorithm is designed to be highly efficient, with performance that is generally O(n log n)
 * for average cases, and O(n²) for worst cases. Various optimizations (like heap sort and insertion
 * sort) are used when the partition size becomes small enough to improve performance.
 * </p>
 *
 * <p>
 * The class implements the {@link IntSorter} interface and allows sorting of an integer array.
 * </p>
 *
 * Threshold constants are defined for various optimizations:
 * <ul>
 * <li>{@link #HEAP_SORT_THRESHOLD} - The size threshold at which heap sort is used instead of
 * quicksort.</li>
 * <li>{@link #NANO_INSERTION_SORT_THRESHOLD} - The threshold for switching to insertion sort for
 * small partitions.</li>
 * <li>{@link #PAIR_INSERTION_SORT_THRESHOLD} - The threshold for switching to a paired insertion
 * sort.</li>
 * <li>{@link #MERGING_SORT_THRESHOLD} - The threshold for switching to a merging sort for very
 * large partitions.</li>
 * <li>{@link #MAX_RECURSION_DEPTH} - Limits recursion depth to prevent stack overflow in the worst
 * case.</li>
 * </ul>
 */
public class QuickSort implements IntSorter {
  ...
  /**
   * The maximum allowed recursion depth, beyond which stack overflow risks are mitigated. Default
   * value is 100.
   */
  private static final int MAX_RECURSION_DEPTH = 100;

  /**
   * A constant that represents the leftmost bits to be used during recursion. This is related to
   * the maximum recursion depth and used in controlling the sorting behavior.
   */
  ...
  /**
   * Returns the comparator used for sorting, or {@code null} if the default natural ordering is
   * used.
   *
  ...
  private static void recursiveSort(int[] block, int bits, int low, int high) {
    ...
    if ((block[e1] < block[e2]) && (block[e2] < block[e3]) && (block[e3] < block[e4])
        && (block[e4] < block[e5])) {
  ...
  /**
   * Performs an optimized version of insertion sort for small arrays or partitions. This method is
   * designed to handle small partitions during the Dual-Pivot Quicksort process. The elements from
   * the left part of the array act as sentinels, which helps to skip expensive checks on the left
   * boundary during the sorting process.
   *
   * @param block the array to sort
  ...
  /**
   * Performs an optimized paired insertion sort for small partitions. In this method, two elements
   * are inserted at once on each iteration. The method first inserts the greater element, and then
   * inserts the smaller element from the position where the greater element was inserted. It also
   * takes advantage of the sentinel elements, which reduces boundary checks.
   *
   * @param block the array to sort
  ...
  /**
   * Performs HeapSort on a specified subarray of the given array. HeapSort is used when the
   * partition size becomes large enough that the efficiency of quicksort begins to degrade. This
   * method builds a heap and performs heapification to sort the array in-place.
   *
   * @param block the array to sort
   * @param left the starting index of the subarray to sort
   * @param right the ending index (exclusive) of the subarray to sort
  ...
  /**
   * Performs the "push down" operation for heapification during HeapSort. This ensures that the
   * heap property is maintained by moving the element at index {@code p} down the heap to its
   * correct position.
   *
  ...
  /**
   * Performs the merging sort algorithm on the specified subarray of the given array. Merging Sort
   * is used when the size of the subarray exceeds a certain threshold. It tries to identify
   * ascending and descending sequences in the array and merge them efficiently.
   *
   * @param block the array to sort
   * @param low the starting index of the subarray to sort
   * @param high the ending index (exclusive) of the subarray to sort
  ...
  /**
   * Merges two subarrays from the specified array into a single sorted array. The method works
   * recursively and splits the array into smaller parts to efficiently merge them.
   *
   ...
   */
  private static int[] merge(int[] block1, int[] block2, boolean isSource, int offset, int[] run,
      int lo, int hi) {
    if (hi - lo == 1) {
      if (isSource == true)
        return block1;
    ...
    return merge((a1 == block1) ? block2 : block1, (a1 == block1) ? run[lo] - offset : run[lo], a1,
        (a1 == block2) ? run[lo] - offset : run[lo], (a1 == block2) ? run[mi] - offset : run[mi],
        a2, (a2 == block2) ? run[mi] - offset : run[mi],
        (a2 == block2) ? run[hi] - offset : run[hi]);
  }

  /**
   * Merges two sorted subarrays into a destination array. This method assumes that both subarrays
   * are sorted and merges them into one sorted array.
   *
   ...
   */
  private static int[] merge(int[] dst, int k, int[] block1, int i, int hi, int[] block2, int j,
      int hj) {
    while (true) {
  ...
  /**
   * Recursively sorts the specified subarray using a specified comparison function. This method is
   * typically used to implement sorting algorithms that need to recursively divide the array into
   * smaller parts and apply sorting to each part.
   *
   * @param block the array to sort
  ...
  /**
   * Performs a nano-insertion sort on the specified subarray of the given array. This method is an
   * optimization of the regular insertion sort used in the context of Quicksort. Elements are
   * inserted in order while minimizing the number of checks by skipping expensive checks for
   * elements in the left part of the array that play the role of sentinels.
   *
   * @param block the array to sort
   * @param low the starting index of the subarray to sort
   * @param high the ending index (exclusive) of the subarray to sort
  ...
  /**
   * Performs a pair-insertion sort on the specified subarray of the given array. This method sorts
   * pairs of elements on each iteration, ensuring that the larger element is inserted first,
   * followed by the smaller element. In the context of Dual-Pivot Quicksort, elements from the left
   * part play the role of sentinels.
   *
   * @param block the array to sort
   * @param left the starting index of the subarray to sort
   * @param right the ending index (exclusive) of the subarray to sort
  ...
  /**
   * Performs a heap sort on the specified subarray of the given array. Heap Sort builds a max-heap
   * and repeatedly extracts the largest element, placing it at the correct position in the array.
   *
   * @param block the array to sort
   * @param left the starting index of the subarray to sort
   * @param right the ending index (exclusive) of the subarray to sort
  ...
  /**
   * Performs the push down operation for heap sorting. This ensures that the heap property is
   * maintained by pushing a value down the heap until it reaches a position where the heap property
   * holds.
   *
   * @param block the array representing the heap
   * @param p the index of the element to push down
  ...
  /**
   * Performs a merging sort on the provided subarray. This method checks if the array is highly
   * structured and attempts to merge runs efficiently if the array is already partially sorted.
   *
   * @param block the array to sort
   * @param low the starting index of the subarray to sort
   * @param high the ending index (exclusive) of the subarray to sort
  ...
  /**
   * Merges two sorted subarrays from the given array into one sorted array. This method is used in
   * a recursive merge process to merge portions of the array.
   *
   * @param block1 the first array to merge
   * @param block2 the second array to merge
   ...
   */
  private static int[] merge(int[] block1, int[] block2, boolean isSource, int offset, int[] run,
      int lo, int hi, ArrayComparator cmp) {
    if (hi - lo == 1) {
      if (isSource == true)
        return block1;
    ...
    return merge((a1 == block1) ? block2 : block1, (a1 == block1) ? run[lo] - offset : run[lo], a1,
        (a1 == block2) ? run[lo] - offset : run[lo], (a1 == block2) ? run[mi] - offset : run[mi],
        a2, (a2 == block2) ? run[mi] - offset : run[mi],
        (a2 == block2) ? run[hi] - offset : run[hi], cmp);
  }

  /**
   * Merges two sorted subarrays into a destination array. This is used during the merge process to
   * combine the elements of the two subarrays into one sorted array.
   *
