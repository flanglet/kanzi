...
/**
 * A fast implementation of the Radix Sort algorithm, which sorts integer or
 * byte arrays using multiple buckets based on radix (binary digit groupings).
 * This implementation uses lists of buckets per radix to efficiently sort data
 * with O(kn) time complexity, where n is the number of keys and k is the number
 * of digits per key.
 *
 * Radix Sort works by distributing the numbers into different buckets based on
 * individual digits (or groups of bits) and then reassembling the numbers in
 * sorted order. This process is repeated for each digit until all the numbers
 * are sorted.
 *
 * <p>
 ...
 * </ul>
 *
 * This class implements both the {@link IntSorter} and {@link ByteSorter}
 * interfaces.
 *
 * @see <a href="http://en.wikipedia.org/wiki/Radix_sort">Wikipedia: Radix
 *      Sort</a>
 *
 *      <p>
 *      Complexity: O(kn), where n is the number of elements and k is the number
 *      of digits in each element (logarithmic to the maximum element size).
 */
public final class RadixSort implements IntSorter, ByteSorter {
    ...
    /**
     * Buffers used to hold the buckets for sorting. Array structure: [number of
     * digits][bucket size]
     */
    private final int[][] buffers;

    /**
     * Constructs a RadixSort with a default radix of 4 bits (radix 16). The radix
     * can be changed later by creating a new instance.
     */
    public RadixSort() {
    ...
    /**
     * Constructs a RadixSort with the specified number of bits per radix. Only
     * values of 4 or 8 bits are supported.
     *
     * @param bitsRadix
     *            the number of bits per radix (either 4 or 8).
     * @throws IllegalArgumentException
     *             if the number of bits is not 4 or 8.
     */
     ...
     * @param blkptr
     *            the starting index for the range to sort.
     * @param count
    ...
    public boolean sort(int[] input, int blkptr, int count) {
        ...
        return (this.bitsRadix == 4) ? this.sort16(input, blkptr, count) : this.sort256(input, blkptr, count);
    }
    ...
    /**
     * Sorts the specified range of bytes in the input array using Radix Sort with a
     * 4-bit radix (radix of 16).
     *
     * @param input
     *            the input array to sort.
     * @param blkptr
    ...
    /**
     * Sorts the specified range of bytes in the input array using a BucketSort for
     * radix 8-bit (radix of 256).
     *
     * @param input
     *            the input array to sort.
     * @param blkptr
