 ...
 * <p>
 * See "Asymmetric Numeral System" by Jarek Duda at http://arxiv.org/abs/0902.0271 Some code has
 * been ported from https://github.com/rygorous/ryg_rans For an alternate C implementation example,
...
public class ANSRangeDecoder implements EntropyDecoder {
  ...
  private static final int ANS_DECODE_CHECK_INTERVAL = 128;
  private static final int ANS_PAYLOAD_GUARD = 2 * ANS_DECODE_CHECK_INTERVAL + 2;
  ...
  /**
   * Creates a new ANSRangeDecoder with a default order of 0 and default chunk size.
   *
   * @param bs The InputBitStream to read compressed data from.
   */
  ...
  /**
   * Creates a new ANSRangeDecoder with the specified order and default chunk size.
   *
   * @param bs The InputBitStream to read compressed data from.
  ...
  /**
   * Creates a new ANSRangeDecoder with the specified order and chunk size.
   *
   * @param bs The InputBitStream to read compressed data from.
   * @param order The order of the ANS model (0 or 1).
  ...
  public ANSRangeDecoder(InputBitStream bs, int order, int chunkSize) {
    ...
    if (chunkSize < MIN_CHUNK_SIZE)
      throw new IllegalArgumentException(
          "ANS Codec: The chunk size must be at least " + MIN_CHUNK_SIZE);
  ...
  /**
   * Creates a new ANSRangeDecoder with the specified context and order.
   *
   * @param bs The InputBitStream to read compressed data from.
   * @param ctx A map containing context information for the decoder.
   * @param order The order of the ANS model (0 or 1).
   */
  ...
  /**
   * Creates a new ANSRangeDecoder with the specified context, order, and chunk size.
   *
   * @param bs The InputBitStream to read compressed data from.
   * @param ctx A map containing context information for the decoder.
   * @param order The order of the ANS model (0 or 1).
  ...
  /**
   * Decodes a chunk of data using version 1 of the ANS decoding algorithm.
   *
   * @param block The byte array to decode into.
  ...
  protected void decodeChunkV1(byte[] block, final int start, final int end) {
    ...
    if (this.order == 0) {
      ...
      for (int i = start; i < end2; i += 2) {
        ...
        // D(x) = (s, q_s (x/M) + mod(x,M) - b_s) where s is such b_s <= x mod M <
        // b_{s+1}
  ...
  /**
   * Decodes a symbol from the ANS state and normalizes the state.
   *
   ...
   * @param mask The mask for the log range.
   * @return The new ANS state after decoding and normalization.
   */
  ...
  /**
   * Decodes a chunk of data using version 2 of the ANS decoding algorithm.
   *
   * @param block The byte array to decode into.
   * @param start The starting position in the block.
   * @param end The ending position in the block.
  ...
  protected boolean decodeChunkV2(byte[] block, final int start, final int end) {
    ...
    final int minBufSize = size + extra + ANS_PAYLOAD_GUARD;

    if (this.buffer.length < minBufSize)
      this.buffer = new byte[minBufSize];

    if (sz > this.buffer.length - ANS_PAYLOAD_GUARD)
      return false;

    // Keep a small, zeroed guard after the declared payload. The hot symbol
    // loop validates payload consumption only at batched boundaries.
    this.bitstream.readBits(this.buffer, 0, 8 * sz);
    Arrays.fill(this.buffer, sz, sz + ANS_PAYLOAD_GUARD, (byte) 0);
    ...
    if (this.order == 0) {
      ...
      for (int batch = start; batch < end4; batch += ANS_DECODE_CHECK_INTERVAL) {
        final int endBatch = Math.min(batch + ANS_DECODE_CHECK_INTERVAL, end4);
      ...
      for (int batch = 0; batch < quarter; batch += ANS_DECODE_CHECK_INTERVAL >> 2) {
        final int endBatch = Math.min(batch + (ANS_DECODE_CHECK_INTERVAL >> 2), quarter);

        for (; i0 < start + endBatch; i0++, i1++, i2++, i3++) {
  ...
  /**
   * Decodes the header of the ANS compressed stream, including the log range, alphabet, and
   * frequencies.
   *
   * @param frequencies A 2D array to store the decoded frequencies for each context and symbol.
   * @param alphabet An array to store the decoded alphabet.
   * @return The total size of the alphabet across all contexts.
   * @throws BitStreamException If an invalid bitstream is encountered (e.g., incorrect range,
   *         frequency, or alphabet size).
   */
  protected int decodeHeader(int[][] frequencies, int[] alphabet) {
    ...
    for (int k = 0; k < dim; k++) {
      ...
      for (int i = 1; i < alphabetSize; i += chkSize) {
        ...
        if (1 << logMax > scale) {
          throw new BitStreamException(
              "Invalid bitstream: incorrect frequency size " + logMax + " in ANS range decoder",
              BitStreamException.INVALID_STREAM);
        }
        ...
        for (int j = i; j < endj; j++) {
          ...
          if ((freq <= 0) || (freq >= scale)) {
            throw new BitStreamException("Invalid bitstream: incorrect frequency " + freq
                + " for symbol '" + alphabet[j] + "' in ANS range decoder",
                BitStreamException.INVALID_STREAM);
          }
      ...
      if (scale <= sum) {
        throw new BitStreamException("Invalid bitstream: incorrect frequency " + f[alphabet[0]]
            + " for symbol '" + alphabet[0] + "' in ANS range decoder",
            BitStreamException.INVALID_STREAM);
      }
  ...
  /**
   * Returns the InputBitStream used by this decoder.
   *
   * @return The InputBitStream.
   */
  @Override
  ...
  /**
   * Represents a symbol with its cumulative frequency and frequency.
   */
  static class Symbol {
    int cumFreq;
    int freq;
    ...
    /**
     * Resets the symbol's cumulative frequency and frequency.
     *
     * @param cumFreq The cumulative frequency.
     * @param freq The frequency.
     * @param logRange The log range.
     */
