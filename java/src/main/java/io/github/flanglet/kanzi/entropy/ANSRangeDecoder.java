 ...
 * http://arxiv.org/abs/0902.0271 Some code has been ported from
 * https://github.com/rygorous/ryg_rans For an alternate C implementation
 * example, see https://github.com/Cyan4973/FiniteStateEntropy
 */
public class ANSRangeDecoder implements EntropyDecoder {
    ...
    /**
     * Creates a new ANSRangeDecoder with a default order of 0 and default chunk
     * size.
     *
     * @param bs
     *            The InputBitStream to read compressed data from.
     */
    ...
    /**
     * Creates a new ANSRangeDecoder with the specified order and default chunk
     * size.
     *
     * @param bs
     *            The InputBitStream to read compressed data from.
     * @param order
     *            The order of the ANS model (0 or 1).
     */
    ...
    /**
     * Creates a new ANSRangeDecoder with the specified context, order, and chunk
     * size.
     *
     * @param bs
     *            The InputBitStream to read compressed data from.
     ...
     *            The starting position in the block.
     * @param end
     *            The ending position in the block.
     */
     ...
     *
     * @param idx
     *            An array containing the current index in the buffer.
     * @param st
     *            The current ANS state.
     * @param sym
     *            The symbol to decode.
     * @param mask
     *            The mask for the log range.
    ...
    /**
     * Decodes the header of the ANS compressed stream, including the log range,
     * alphabet, and frequencies.
     *
     * @param frequencies
     *            A 2D array to store the decoded frequencies for each context and
     *            symbol.
     * @param alphabet
     *            An array to store the decoded alphabet.
     * @return The total size of the alphabet across all contexts.
     * @throws BitStreamException
     *             If an invalid bitstream is encountered (e.g., incorrect range,
    ...
    protected int decodeHeader(int[][] frequencies, int[] alphabet) {
        ...
        if ((this.logRange < 8) || (this.logRange > 16))
            throw new BitStreamException("Invalid bitstream: range = " + this.logRange + " (must be in [8..16])",
        ...
        for (int k = 0; k < dim; k++) {
                ...
                for (int j = i; j < endj; j++) {
                    ...
                    if ((freq <= 0) || (freq >= scale)) {
                        throw new BitStreamException("Invalid bitstream: incorrect frequency " + freq + " for symbol '"
                                + alphabet[j] + "' in ANS range decoder", BitStreamException.INVALID_STREAM);
                    }
            ...
            if (scale <= sum) {
                throw new BitStreamException("Invalid bitstream: incorrect frequency " + f[alphabet[0]]
                        + " for symbol '" + alphabet[0] + "' in ANS range decoder", BitStreamException.INVALID_STREAM);
            }
