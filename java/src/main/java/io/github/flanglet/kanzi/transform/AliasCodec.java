...
/**
 * The AliasCodec class implements a byte transform that aims to compress data by substituting
 * frequently occurring patterns with shorter representations. It supports both forward and inverse
 * transformations.
 * <p>
...
public class AliasCodec implements ByteTransform {
  ...
  /**
   * Default constructor. Creates an AliasCodec instance with no specific context and not restricted
   * to DNA data.
   */
  public AliasCodec() {
    this.ctx = null;
  ...
  /**
   * Constructor with DNA restriction flag. Creates an AliasCodec instance with no specific context
   * and optionally restricted to DNA data.
   *
  ...
  /**
   * Constructor with context. Creates an AliasCodec instance with a specific context.
   *
  ...
  public boolean forward(SliceByteArray input, SliceByteArray output) {
    ...
    if (this.ctx != null) {
      ...
      if ((this.onlyDNA == true) && (dt != Global.DataType.UNDEFINED)
          && (dt != Global.DataType.DNA))
        return false;
    }
    ...
    if (n0 >= 240) {
      ...
      if (n0 == 255) {
        ...
        if (n0 >= 252) {
          ...
          while (srcIdx < count) {
            dst[dstIdx++] =
                (byte) ((map8[src[srcIdx + 0] & 0xFF] << 6) | (map8[src[srcIdx + 1] & 0xFF] << 4)
  ...
  public boolean inverse(SliceByteArray input, SliceByteArray output) {
    ...
    if (n >= 240) {
      ...
      for (int i = 0; i < n; i++) {
        map16[src[srcIdx + 2] & 0xFF] =
            0x20000 | (src[srcIdx] & 0xFF) | ((src[srcIdx + 1] & 0xFF) << 8);
        srcIdx += 3;
      }
  ...
  /**
   * The Alias class represents a mapping between a symbol and its frequency. It is used to
   * facilitate the aliasing process in the transformation.
   */
