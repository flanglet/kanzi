...
/**
 * The {@code HeapSort} class implements the heap sort algorithm, a comparison-based sorting algorithm with an average and
 * worst-case time complexity of O(n log n).
 *
 * <p>Heap sort works by first building a binary heap from the input data, and then repeatedly extracting the maximum
 * (or minimum) element from the heap and reconstructing the heap. Although heap sort has O(n log n) time complexity, it is
 * often slower in practice compared to other O(n log n) algorithms such as QuickSort, due to larger constant factors.</p>
 *
 * <p>This implementation allows an optional custom comparator to be used for comparing array elements. If no comparator is
 * provided, the natural ordering of the elements is used.</p>
 *
...
public final class HeapSort implements IntSorter {
    ...
    /**
     * Constructs a {@code HeapSort} instance without a custom comparator.
    ...
    /**
     * Constructs a {@code HeapSort} instance with the specified comparator.
    ...
    /**
     * Performs a single heap sort operation on the portion of the array specified by {@code blkptr}, {@code idx}, and {@code count}.
     * This method ensures that the subtree rooted at {@code idx} is a valid heap.
     *
