...
/**
 * The {@code HeapSort} class implements the heap sort algorithm, a comparison-based sorting
 * algorithm with an average and worst-case time complexity of O(n log n).
 *
 * <p>
 * Heap sort works by first building a binary heap from the input data, and then repeatedly
 * extracting the maximum (or minimum) element from the heap and reconstructing the heap. Although
 * heap sort has O(n log n) time complexity, it is often slower in practice compared to other O(n
 * log n) algorithms such as QuickSort, due to larger constant factors.
 * </p>
 *
 * <p>
 * This implementation allows an optional custom comparator to be used for comparing array elements.
...
public final class HeapSort implements IntSorter {
  ...
  /**
   * Constructs a {@code HeapSort} instance without a custom comparator. This will use the natural
  ...
  /**
   * Constructs a {@code HeapSort} instance with the specified comparator. If {@code cmp} is
  ...
  /**
   * Performs a single heap sort operation on the portion of the array specified by {@code blkptr},
   * {@code idx}, and {@code count}. This method ensures that the subtree rooted at {@code idx} is a
   * valid heap.
   *
   * @param array the array to be sorted.
   * @param blkptr the starting index of the array to be sorted.
   * @param idx the index of the current node to heapify.
   * @param count the total number of elements in the heap.
   * @param cmp the comparator used for comparisons, or {@code null} to use natural ordering.
   */
