...
public class HuffmanDecoder implements EntropyDecoder {
    ...
    /**
     * Reads the code lengths from the bitstream and generates the Huffman codes for
     * decoding.
     *
    ...
    public int readLengths() throws BitStreamException {
        ...
        if (HuffmanCommon.generateCanonicalCodes(this.sizes, this.codes, this.alphabet, count,
            ...
            throw new BitStreamException(
                    "Could not generate Huffman codes: max code length (" + this.maxSymbolSize + " bits) exceeded",
                    BitStreamException.INVALID_STREAM);
        }
    ...
    /**
     * Builds the decoding tables used for efficient symbol lookup. The table maps
     * prefixes of Huffman codes to their corresponding symbol and code length.
     *
     * @param count
     ...
     *            The starting position in the block.
     * @param count
     *            The number of bytes to decode in this chunk.
    ...
    public boolean decodeChunk(byte[] block, int blkptr, int count) {
        ...
        while (n < szFrag - 4) {
            ...
            state0 = (state0 << shift0) | (Memory.BigEndian.readLong64(this.buffer, idx0) >>> (63 - shift0) >>> 1); // handle
                                                                                                                    // shift
                                                                                                                    // =
                                                                                                                    // 0
            state1 = (state1 << shift1) | (Memory.BigEndian.readLong64(this.buffer, idx1) >>> (63 - shift1) >>> 1); // handle
                                                                                                                    // shift
                                                                                                                    // =
                                                                                                                    // 0
            state2 = (state2 << shift2) | (Memory.BigEndian.readLong64(this.buffer, idx2) >>> (63 - shift2) >>> 1); // handle
                                                                                                                    // shift
                                                                                                                    // =
                                                                                                                    // 0
            state3 = (state3 << shift3) | (Memory.BigEndian.readLong64(this.buffer, idx3) >>> (63 - shift3) >>> 1); // handle
                                                                                                                    // shift
                                                                                                                    // =
                                                                                                                    // 0
