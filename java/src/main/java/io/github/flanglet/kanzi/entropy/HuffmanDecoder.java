...
/**
 * <p>
 * Implementation of a Huffman decoder. This class decodes symbols from a
 * bitstream using Huffman codes, which are variable-length codes optimized
 * for symbol frequencies.
 * </p>
 *
 * <p>
 * The decoder uses a lookup table to efficiently map incoming bits to symbols.
 * It supports different bitstream versions and handles chunk-based decoding.
 * </p>
 *
 * <p>
...
public class HuffmanDecoder implements EntropyDecoder {
   /**
    * A mask used to extract the relevant bits for table lookup in bitstream
    * version 4.
    */
    ...
    * @param ctx       A map containing additional context information for
    *                  decompression.
    * @throws BitStreamException       if an error occurs during bitstream
    *                                  operations.
   ...
   /**
    * Reads the code lengths from the bitstream and generates the Huffman codes
    * for decoding.
    *
    * @return The number of symbols in the alphabet, or 0 if the alphabet is empty.
    * @throws BitStreamException if the bitstream contains invalid data, such as
    *                            incorrect Huffman symbol or code size.
    */
   ...
   /**
    * Builds the decoding tables used for efficient symbol lookup.
    * The table maps prefixes of Huffman codes to their corresponding symbol and
    * code length.
    *
    ...
    * into the provided byte array. It handles different bitstream versions.
    * </p>
    *
    ...
    * <p>
    * This method processes data in chunks, reading code lengths, rebuilding codes,
    * and then decoding the compressed data.
    * </p>
    *
    * @param block  The byte array to decode into.
    * @param blkptr The starting position in the block.
      ...
      while (startChunk < end) {
         ...
         if (szBits != 0) {
            ...
            while (idx < sz - 8) {
               ...
               state = (state << shift) | (Memory.BigEndian.readLong64(this.buffer, idx) >>> (63 - shift) >>> 1); // handle
                                                                                                                  // shift
                                                                                                                  // = 0
    ...
    * <p>
    * This method processes data in chunks, reading code lengths, rebuilding codes,
    * and then decoding the compressed data using a parallel approach for larger
    * chunks.
    * </p>
    *
    * @param block  The byte array to decode into.
    ...
    * <p>
    * This method is optimized for larger chunks (count >= 32) and processes four
    * parallel streams of compressed data.
    * </p>
    *
    * @param block  The byte array to decode into.
    * @param blkptr The starting position in the block.
    * @param count  The number of bytes to decode in this chunk.
    * @return {@code true} if the chunk was decoded successfully, {@code false}
    *         otherwise.
    */
