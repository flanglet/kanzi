 ...
 * encoding/decoding, frequency normalization, and variable-length integer
 * encoding/decoding.
 */
public class EntropyUtils {
    ...
    /**
     * Decodes an alphabet from the input bitstream. The alphabet can be full (256
     * symbols), empty (0 symbols), or partial. For partial alphabets, a bitmask is
     * read to reconstruct the symbols.
     *
     * @param ibs
     *            The input bitstream to read the encoded alphabet from.
     * @param alphabet
     *            An array to store the decoded symbols.
     * @return The number of symbols decoded.
     * @throws BitStreamException
     *             If the bitstream is invalid (e.g., incorrect alphabet size).
     */
    ...
    /**
     * Normalizes frequencies of symbols in an alphabet to a given scale. This
     * method adjusts the frequencies so that their sum equals the specified scale,
     * while preserving the relative proportions as much as possible. It also
     * populates the alphabet array with the present symbols.
     * <p>
     * This method is not thread safe.
     * </p>
     *
     * @param freqs
     *            An array containing the frequencies of symbols. This array will be
     *            updated with normalized frequencies.
     * @param alphabet
     *            An array to store the symbols present in the alphabet. This array
     *            will be updated.
     * @param totalFreq
     *            The sum of the original frequencies.
     * @param scale
     *            The target sum for the normalized frequencies.
     * @return The size of the alphabet (number of unique symbols with non-zero
     *         frequency).
     * @throws IllegalArgumentException
     *             If the alphabet size or scale parameter is invalid.
     */
    public static int normalizeFrequencies(int[] freqs, int[] alphabet, int totalFreq, int scale) {
        if (alphabet.length > 256)
            throw new IllegalArgumentException(
                    "Invalid alphabet size parameter: " + alphabet.length + " (must be less than or equal to 256)");

        if ((scale < 1 << 8) || (scale > 1 << 16))
            throw new IllegalArgumentException("Invalid scale parameter: " + scale + " (must be in [256..65536])");
     ...
     *
     * @param bs
     *            The output bitstream to write to.
     * @param value
     ...
     *
     * @param bs
     *            The input bitstream to read from.
