...
public final class TextCodec implements ByteTransform {
  ...
  public static int computeStats(byte[] block, final int start, final int end, int[] freqs0,
      boolean strict) {
  ...
  private static boolean sameWords(byte[] buf1, final int idx1, byte[] buf2, final int idx2,
      int length) {
    while (length > 0) {
      length--;
  ...
  static class TextCodec1 implements ByteTransform {
    ...
    private void reset(int count) {
      ...
      if (this.dictList.length < this.dictSize) {
        ...
        this.dictList[STATIC_DICT_WORDS] =
            new DictEntry(new byte[] {ESCAPE_TOKEN2}, 0, 0, STATIC_DICT_WORDS, 1);
        this.dictList[STATIC_DICT_WORDS + 1] =
            new DictEntry(new byte[] {ESCAPE_TOKEN1}, 0, 0, STATIC_DICT_WORDS + 1, 1);
      }
      ...
      for (int i = srcIdx; i < srcEnd; i++) {
        ...
        switch (cur) {
          ...
          case ESCAPE_TOKEN2:
            ...
            final int idx =
                (cur == ESCAPE_TOKEN1) ? this.staticDictSize - 1 : this.staticDictSize - 2;
            int lenIdx = 2;
