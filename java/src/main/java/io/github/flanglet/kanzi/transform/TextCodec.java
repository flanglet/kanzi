...
public final class TextCodec implements ByteTransform {
    ...
    private static final byte[] DICT_EN_1024 = ("TheBeAndOfInToWithItThatForYouHeHaveOnSaidSayAtButWeByHadTheyAsW"
    ...
    public static int computeStats(byte[] block, final int start, final int end, int[] freqs0, boolean strict) {
        ...
        if (notText == false) {
            if (strict == true)
                notText = ((nbTextChars < (count >> 2)) || (freqs0[0] >= (count / 100))
                        || ((nbASCII / 95) < (count / 100)));
            else
        ...
        if (nbBinChars <= count - count / 10) {
            ...
            // and 'high enough'. Also check it is worth to attempt replacing ampersand
            // sequences.
            // Getting this flag wrong results in a very small compression speed
            // degradation.
            final int f1 = freqs0['<'];
            final int f2 = freqs0['>'];
    ...
    static class TextCodec1 implements ByteTransform {
        ...
        private void reset(int count) {
            ...
            if (this.dictList.length < this.dictSize) {
                ...
                System.arraycopy(STATIC_DICTIONARY, 0, this.dictList, 0,
                        Math.min(STATIC_DICTIONARY.length, this.dictSize));
                ...
                this.dictList[STATIC_DICT_WORDS + 1] = new DictEntry(new byte[]{ESCAPE_TOKEN1}, 0, 0,
                        STATIC_DICT_WORDS + 1, 1);
            }
        ...
        public boolean forward(SliceByteArray input, SliceByteArray output) {
            ...
            if (this.ctx != null) {
                ...
                // Filter out most types. Still check binaries which may contain significant
                // parts of text
        ...
        private int emitSymbols(byte[] src, final int srcIdx, byte[] dst, int dstIdx, final int srcEnd,
                final int dstEnd) {
