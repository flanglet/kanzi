...
public class ROLZCodec implements ByteTransform {
    ...
    /**
     * The size of the chunk to process at a time. This is used to limit memory
    ...
    /**
     * Creates a new {@link ROLZCodec} instance. The default implementation uses
     * ROLZCodec1 (ANS based).
     */
    public ROLZCodec() {
     ...
     *
     * @param extra
     *            If {@code true}, uses ROLZCodec2 (CM based), otherwise uses
     *            ROLZCodec1 (ANS based).
     */
    ...
    /**
     * Creates a new {@link ROLZCodec} instance with the given context. The choice
     * between ROLZCodec1 and ROLZCodec2 is based on the "transform" key in the
     * context.
     *
     * @param ctx
     *            A map of parameters to configure the ROLZ codec.
     */
    ...
    /**
     * Emits a copy of bytes from a reference position to the destination. This
     * method handles overlapping copies efficiently.
     *
     * @param dst
     ...
     * @param ref
     *            The starting index of the reference in the destination array.
     * @param matchLen
     *            The length of the match to copy.
    ...
    /**
     * <p>
     * Encodes the provided data in the source slice and puts the result in the
     * destination slice.
     * </p>
     *
     * @param src
     *            The source slice of bytes.
     * @param dst
     *            The destination slice of bytes.
    ...
    /**
     * <p>
     * Decodes the provided data in the source slice and puts the result in the
     * destination slice.
     * </p>
     *
     * @param src
    ...
    /**
     * <p>
     * Implementation of a ROLZ codec using ANS for encoding/decoding literals and
     * matches.
     * </p>
     */
    ...
    static class ROLZCodec1 implements ByteTransform {
        ...
        /**
         * Counters for each hash key, used to track the number of times a key has been
         * seen.
         */
        ...
        /**
         * Creates a new {@link ROLZCodec1} instance with the specified logarithm of
         * position checks.
         * 
         * @param logPosChecks
         *            The logarithm base 2 of the number of position checks.
         ...
         *
         * @param sba
         *            The source slice byte array.
         * @param pos
         *            The current position in the source buffer.
         * @param hash32
         *            The 32-bit hash of the current position.
         * @param counter
         *            The current counter for the hash key.
         * @param base
         *            The base index in the matches array for the current hash key.
         * @return An integer containing the best match index (upper 16 bits) and length
         *         (lower 16 bits), or -1 if no match is found.
         */

        // return position index (LOG_POS_CHECKS bits) + length (16 bits) or -1
        private int findMatch(final SliceByteArray sba, final int pos, int hash32, int counter, int base) {
         ...
         *
         * @param lenBuf
         *            The slice byte array to write the length to.
         * @param length
         *            The length to emit.
         */
                ...
                while (dstIdx < endChunk) {
                    ...
                    if (litLen > 0) {
                        ...
                        for (int j = 0; j < litLen; j++) {
                            final int key = (mm == MIN_MATCH3)
                                    ? getKey1(dst, dstIdx + j - dt)
         ...
         *
         * @param lenBuf
         *            The slice byte array to read the length from.
    ...
    /**
     * <p>
     * Implementation of a ROLZ codec using CM (Context Model) for encoding/decoding
    ...
    static class ROLZCodec2 implements ByteTransform {
        ...
        /**
         * Creates a new {@link ROLZCodec2} instance with the specified logarithm of
         * position checks.
         * 
         * @param logPosChecks
         *            The logarithm base 2 of the number of position checks.
         ...
         *            The current position in the source buffer.
         * @param key
         *            The hash key for the current position.
         * @return An integer containing the best match index (upper 16 bits) and length
         *         (lower 16 bits), or -1 if no match is found.
         */
        ...
        public boolean forward(SliceByteArray input, SliceByteArray output) {
            ...
            while (startChunk < srcEnd) {
                ...
                while (srcIdx < endChunk) {
                    ...
                    final int match = (mm == MIN_MATCH3)
                            ? findMatch(sba2, srcIdx, getKey1(src, srcIdx - dt))
    ...
    static class ROLZEncoder {
         ...
         *
         * @param litLogSize
         *            The logarithm base 2 of the context size for literals.
         * @param mLogSize
         *            The logarithm base 2 of the context size for matches.
         * @param sba
         *            The slice byte array to write encoded data to.
         */
         ...
         *
         * @param n
         *            The index of the probability table to use (LITERAL_CTX or
         *            MATCH_CTX).
         * @param ctx
         *            The context value.
         */
         ...
         *
         * @param val
         *            The integer containing the bits to encode.
         * @param n
         *            The number of bits to encode (from LSB to MSB).
         */
        ...
        public void encodeBit(int bit) {
            ...
            if (bit == 0) {
                ...
                this.probs[this.pIdx][this.ctx
                        + this.c1] -= (((this.probs[this.pIdx][this.ctx + this.c1] - 0xFFFF) >> 5) + 1);
                this.c1 += (this.c1 + 1);
            }
    ...
    static class ROLZDecoder {
         ...
         *            The logarithm base 2 of the context size for matches.
         * @param sba
         *            The slice byte array to read encoded data from.
         */
        ...
        public int decodeBit() {
            // Calculate interval split
            final long mid = this.low
                    + ((((this.high - this.low) >>> 4) * (this.probs[this.pIdx][this.ctx + this.c1] >>> 4)) >>> 8);
            int bit;
