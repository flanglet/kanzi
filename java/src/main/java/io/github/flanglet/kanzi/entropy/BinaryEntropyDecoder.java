 ...
 * probability of the next bit. The predictor provides the probability for each bit, and the decoder
 * uses this to narrow down the range.
 * </p>
 * <p>
 * The decoding process involves reading bits from an {@link InputBitStream}, updating the range,
 * and normalizing the range by reading more bits when the range becomes too small.
 * </p>
 * <p>
 * This decoder is designed to work with a {@link Predictor} to adaptively decode binary data.
 * </p>
 */
public class BinaryEntropyDecoder implements EntropyDecoder {
  ...
  private long high;
  /**
   * The current value read from the bitstream, representing a point within the range.
   */
   ...
   * <p>
   * The decoder is initialized with an {@link InputBitStream} to read compressed data and a
   * {@link Predictor} to provide probability estimates for decoding bits.
   * </p>
   *
   ...
   * @throws NullPointerException if {@code bitstream} or {@code predictor} is {@code null}.
   */
   ...
   * <p>
   * This method decodes 8 bits sequentially, using the predictor for each bit, to reconstruct a
   * byte.
   * </p>
   *
  ...
  public final byte decodeByte() {
    return (byte) ((this.decodeBit(this.predictor.get()) << 7)
        | (this.decodeBit(this.predictor.get()) << 6) | (this.decodeBit(this.predictor.get()) << 5)
        | (this.decodeBit(this.predictor.get()) << 4) | (this.decodeBit(this.predictor.get()) << 3)
        | (this.decodeBit(this.predictor.get()) << 2) | (this.decodeBit(this.predictor.get()) << 1)
   ...
   * <p>
   * The range is split according to the prediction, and the bit is determined by the current value.
   * The predictor is then updated with the decoded bit.
   * </p>
   *
