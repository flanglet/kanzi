 ...
 * listeners, and multi-threading support. It processes the data in blocks and supports a variety of
 * compression and transformation techniques based on the provided context.
 * <p>
 * The class includes an {@link OutputBitStream} for writing the compressed data, along with a
 * number of other internal buffers and hashers to manage the compression process efficiently.
 * <p>
 ...
 *     new CompressedOutputStream(new FileOutputStream("output.bin"), ctx)) {
...
public class CompressedOutputStream extends OutputStream {
  ...
  /**
   * Constructs a {@link CompressedOutputStream} using a custom output bitstream and provided
   * context.
   *
  ...
  public CompressedOutputStream(OutputBitStream obs, Map<String, Object> ctx) {
    ...
    if (bSize < MIN_BITSTREAM_BLOCK_SIZE)
      throw new IllegalArgumentException(
          "The block size must be at least " + MIN_BITSTREAM_BLOCK_SIZE);
    ...
    if ((tasks > 1) && (threadPool == null))
      throw new IllegalArgumentException(
          "The thread pool cannot be null when the number of jobs is " + tasks);
  ...
  /**
   * Writes the header information for the compressed data stream. This includes metadata such as
   * the compression format version, block sizes, and other relevant parameters that the
   * decompressor will need to interpret the stream.
   *
   ...
   * to this output stream. The general contract for <code>write(array, off, len)</code> is that
   * some of the bytes in the array <code>array</code> are written to the output stream in order;
   * element <code>array[off]</code> is the first byte written and <code>array[off+len-1]</code> is
   ...
   * <p>
   * If <code>array</code> is <code>null</code>, a <code>NullPointerException</code> is thrown.
   * <p>
   * If <code>off</code> is negative, or <code>len</code> is negative, or <code>off+len</code> is
   * greater than the length of the array <code>array</code>, then an IndexOutOfBoundsException is
   * thrown.
   *
   * @param data the data.
   * @param off the start offset in the data.
   * @param len the number of bytes to write.
   * @exception IOException if an I/O error occurs. In particular, an <code>KanziIOException</code>
  ...
  public void write(byte[] data, int off, int len) throws IOException {
    ...
    while (remaining > 0) {
      ...
      final int lenChunk =
          Math.min(remaining, this.bufferThreshold - this.buffers[this.bufferId].index);

      if (lenChunk > 0) {
        // Process a chunk of in-buffer data. No access to bitstream required
        System.arraycopy(data, off, this.buffers[this.bufferId].array,
        ...
        if (this.buffers[this.bufferId].index >= this.bufferThreshold) {
          ...
          final int nbTasks =
              (this.nbInputBlocks == 0) ? this.jobs : Math.min(this.nbInputBlocks, this.jobs);
  ...
  /**
   * Processes a block of data, compressing it according to the configured settings. This method is
   * typically called when the input buffer is full and the compressed output needs to be written.
   *
    ...
    try {
      ...
      for (int taskId = 0; taskId < nbTasks; taskId++) {
        ...
        Callable<Status> task =
            new EncodingTask(this.buffers[taskId], this.buffers[this.jobs + taskId], dataLength,
                this.transformType, this.entropyType, firstBlockId + taskId + 1, this.obs,
  ...
  static class EncodingTask implements Callable<Status> {
     ...
     */
    EncodingTask(SliceByteArray iBuffer, SliceByteArray oBuffer, int length, long transformType,
        int entropyType, int blockId, OutputBitStream obs, XXHash32 hasher32, XXHash64 hasher64,
        AtomicInteger processedBlockId, Listener[] listeners, Map<String, Object> ctx) {
      this.data = iBuffer;
      this.buffer = oBuffer;
      this.length = length;
    ...
    /**
     * Encodes the data block by applying the specified transformations and entropy encoding. This
     * method is executed by the {@link ExecutorService} and returns a {@link Status} object
     * indicating the result of the encoding process.
     *
    ...
    public Status call() throws Exception {
      return this.encodeBlock(this.data, this.buffer, this.length, this.transformType,
          this.entropyType, this.blockId);
    }

    /**
     * Encodes the block of data using the specified transformation and entropy encoding types. The
     * encoding process may involve applying a series of transforms followed by entropy coding.
     *
     ...
     * <li>If there are 4 or fewer transforms, the lower 4 bits (0b0000yyyy) specify the transform
     * skip flags.</li>
     * <li>If there are more than 4 transforms, the next byte (0b00000000) specifies skip
     * flags.</li>
     * </ul>
     *
     ...
     */
    private Status encodeBlock(SliceByteArray data, SliceByteArray buffer, int blockLength,
      ...
      try {
        ...
        if (this.listeners.length > 0) {
          ...
          Event evt = new Event(Event.Type.BEFORE_TRANSFORM, currentBlockId, blockLength, checksum,
              hashType);
        ...
        final int dataSize =
            (postTransformLength < 256) ? 1 : (Global.log2(postTransformLength) >> 3) + 1;
        ...
        if (this.listeners.length > 0) {
          ...
          Event evt = new Event(Event.Type.AFTER_TRANSFORM, currentBlockId, postTransformLength,
              checksum, hashType);
          notifyListeners(this.listeners, evt);
        }

        final int bufSize =
            Math.max(256 * 1024, Math.max(postTransformLength, blockLength + (blockLength >> 3)));
        ...
        if (this.listeners.length > 0) {
          ...
          Event evt = new Event(Event.Type.BEFORE_ENTROPY, currentBlockId, postTransformLength,
              checksum, hashType);
          notifyListeners(this.listeners, evt);
        }
        ...
        if (this.listeners.length > 0) {
          ...
          Event evt1 = new Event(Event.Type.AFTER_ENTROPY, currentBlockId, (written + 7) >> 3,
              checksum, hashType);
          ...
          if (v >= 5) {
            ...
            String msg =
                String.format("{ \"type\":\"%s\", \"id\": %d, \"offset\":%d, \"skipFlags\":%s }",
  ...
  /**
   * A static class representing the status of a block during the compression process. It
   * encapsulates information about the block ID, the error code (if any), and an optional message
   * that provides additional details about the status of the block.
   */
  static class Status {
     ...
     * @param error The error code. A value of 0 indicates no error (OK), any other value indicates
     *        an error.
     * @param msg An optional message that provides additional context or details about the status
     *        of the block.
     */
  ...
  /**
   * A custom subclass of {@link ByteArrayOutputStream} that allows for using a pre-existing byte
   * buffer and ensures that the buffer is large enough to accommodate the specified size.
   *
  ...
  static class CustomByteArrayOutputStream extends ByteArrayOutputStream {
     ...
     *
     * @param buffer The byte array to use as the internal buffer. The buffer length must be greater
     *        than or equal to the specified size.
     * @param size The initial size of the output stream. This must not exceed the length of the
     *        buffer.
     *
     * @throws IllegalArgumentException If the provided buffer length is smaller than the specified
     *         size.
     */
