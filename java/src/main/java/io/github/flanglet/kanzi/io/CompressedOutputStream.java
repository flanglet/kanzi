...
/**
 * A custom {@link OutputStream} that performs data compression with
 * configurable block sizes, listeners, and multi-threading support. It
 * processes the data in blocks and supports a variety of compression and
 * transformation techniques based on the provided context.
 * <p>
 * The class includes an {@link OutputBitStream} for writing the compressed
 * data, along with a number of other internal buffers and hashers to manage the
 * compression process efficiently.
 * <p>
 * Compression is performed in blocks, and listeners can be registered to
 * monitor the process.
 *
 * <p>
 * Example usage:
...
public class CompressedOutputStream extends OutputStream {
    ...
    /**
     * Constructs a {@link CompressedOutputStream} using the default output
     * bitstream and provided context.
     *
     * @param os
     *            The underlying output stream to which the compressed data will be
     *            written.
     * @param ctx
     *            The context map that holds configuration settings for the
     *            compression process.
     */
    ...
    /**
     * Constructs a {@link CompressedOutputStream} using a custom output bitstream
     * and provided context.
     *
     * @param obs
     *            The custom output bitstream to be used for writing compressed
     *            data.
     * @param ctx
     *            The context map that holds configuration settings for the
     *            compression process.
     * @throws NullPointerException
     *             if a null parameter has been provided.
     * @throws IllegalArgumentException
     *             if an invalid parameter has been provided.
     */
    public CompressedOutputStream(OutputBitStream obs, Map<String, Object> ctx) {
        ...
        if (bSize > MAX_BITSTREAM_BLOCK_SIZE)
            throw new IllegalArgumentException(
                    "The block size must be at most " + (MAX_BITSTREAM_BLOCK_SIZE >> 20) + " MiB");
     ...
     * metadata such as the compression format version, block sizes, and other
     * relevant parameters that the decompressor will need to interpret the stream.
     *
     * @throws IOException
     *             If an I/O error occurs while writing the header.
     */
    protected void writeHeader() throws IOException {
        ...
        if (this.obs.writeBits(BITSTREAM_TYPE, 32) != 32)
            throw new KanziIOException("Cannot write bitstream type to header", Error.ERR_WRITE_FILE);

        if (this.obs.writeBits(BITSTREAM_FORMAT_VERSION, 4) != 4)
            throw new KanziIOException("Cannot write bitstream version to header", Error.ERR_WRITE_FILE);
        ...
        if (this.obs.writeBits(chkSize, 2) != 2)
            throw new KanziIOException("Cannot write checksum type to header", Error.ERR_WRITE_FILE);
        ...
        if (this.obs.writeBits(this.transformType, 48) != 48)
            throw new KanziIOException("Cannot write transform types to header", Error.ERR_WRITE_FILE);
        ...
        if (this.obs.writeBits(szMask, 2) != 2)
            throw new KanziIOException("Cannot write size of input to header", Error.ERR_WRITE_FILE);
     ...
     * @return {@code true} if the listener was added successfully, {@code false}
     *         otherwise.
     */
     ...
     * @return {@code true} if the listener was removed successfully, {@code false}
     *         otherwise.
     */
     ...
     * <code>write(array, off, len)</code> is that some of the bytes in the array
     * <code>array</code> are written to the output stream in order; element
     * <code>array[off]</code> is the first byte written and
     * <code>array[off+len-1]</code> is the last byte written by this operation.
     * <p>
     ...
     *                <code>KanziIOException</code> is thrown if the output stream
     *                is closed.
     */
    @Override
    public void write(byte[] data, int off, int len) throws IOException {
        ...
        while (remaining > 0) {
            ...
            if (lenChunk > 0) {
                ...
                System.arraycopy(data, off, this.buffers[this.bufferId].array, this.buffers[this.bufferId].index,
                        lenChunk);
    ...
    /**
     * Processes a block of data, compressing it according to the configured
     * settings. This method is typically called when the input buffer is full and
     * the compressed output needs to be written.
     *
     * @throws IOException
     *             If an I/O error occurs while processing the block.
     */
    private void processBlock() throws IOException {
        ...
        try {
            ...
            // Assign optimal number of tasks and jobs per task (if the number of blocks is
            // known)
            if (nbTasks > 1) {
            ...
            for (int taskId = 0; taskId < nbTasks; taskId++) {
                ...
                Callable<Status> task = new EncodingTask(this.buffers[taskId], this.buffers[this.jobs + taskId],
                        dataLength, this.transformType, this.entropyType, firstBlockId + taskId + 1, this.obs,
                        this.hasher32, this.hasher64, this.blockId, blockListeners, map);
                tasks.add(task);
            }
            ...
            int errorCode = (e instanceof BitStreamException)
                    ? ((BitStreamException) e).getErrorCode()
                    : Error.ERR_UNKNOWN;
     ...
     *
     * @param listeners
     *            An array of listeners to be notified.
     * @param evt
     *            The event to be passed to each listener.
     */
    ...
    /**
     * A task used to encode a block of data. This task can run in parallel with
     * other encoding tasks to perform transformations and entropy encoding on the
     * data.
     * <p>
     * Each encoding task processes a block of data, applies transformations and
     * entropy encoding according to the specified parameters, and produces a status
     * indicating whether the encoding was successful or encountered an error.
     * <p>
     * This class implements the {@link Callable} interface and is intended to be
     * executed by an {@link ExecutorService} to perform concurrent block encoding.
     */
    static class EncodingTask implements Callable<Status> {
         ...
         *
         * @param iBuffer
         *            The input data slice to be encoded.
         * @param oBuffer
         *            The output buffer where the encoded data will be stored.
         * @param length
         *            The length of the input data to encode.
         * @param transformType
         *            The transformation type to apply.
         * @param entropyType
         *            The entropy encoding type to use.
         * @param blockId
         *            The unique ID of the block.
         * @param obs
         *            The output bit stream used for writing the encoded data.
         * @param hasher32
         *            A 32-bit hash function for hashing the data.
         * @param hasher64
         *            A 64-bit hash function for hashing the data.
         * @param processedBlockId
         *            An atomic integer tracking the processed block ID.
         * @param listeners
         *            The array of listeners to notify of events.
         * @param ctx
         *            The context map providing configuration for encoding.
         */
        EncodingTask(SliceByteArray iBuffer, SliceByteArray oBuffer, int length, long transformType, int entropyType,
                int blockId, OutputBitStream obs, XXHash32 hasher32, XXHash64 hasher64, AtomicInteger processedBlockId,
                Listener[] listeners, Map<String, Object> ctx) {
            this.data = iBuffer;
            this.buffer = oBuffer;
            this.length = length;
        ...
        /**
         * Encodes the data block by applying the specified transformations and entropy
         * encoding. This method is executed by the {@link ExecutorService} and returns
         * a {@link Status} object indicating the result of the encoding process.
         *
        ...
        public Status call() throws Exception {
            return this.encodeBlock(this.data, this.buffer, this.length, this.transformType, this.entropyType,
                    this.blockId);
        }

        /**
         * Encodes the block of data using the specified transformation and entropy
         * encoding types. The encoding process may involve applying a series of
         * transforms followed by entropy coding.
         *
         ...
         * <li>If there are 4 or fewer transforms, the lower 4 bits (0b0000yyyy) specify
         * the transform skip flags.</li>
         * <li>If there are more than 4 transforms, the next byte (0b00000000) specifies
         * skip flags.</li>
         * </ul>
         *
         * @param data
         *            The input data slice to be encoded.
         * @param buffer
         *            The output buffer to store the encoded data.
         * @param blockLength
         *            The length of the data block to encode.
         * @param blockTransformType
         *            The transformation type for the block.
         * @param blockEntropyType
         *            The entropy encoding type for the block.
         * @param currentBlockId
         *            The unique ID of the block being processed.
         *
         * @return A {@link Status} object indicating the result of the block encoding.
         */
        private Status encodeBlock(SliceByteArray data, SliceByteArray buffer, int blockLength, long blockTransformType,
            ...
            try {
                ...
                if (this.listeners.length > 0) {
                    ...
                    Event evt = new Event(Event.Type.BEFORE_TRANSFORM, currentBlockId, blockLength, checksum, hashType);
                ...
                if (blockLength <= SMALL_BLOCK_SIZE) {
                    ...
                    if (skipBlockOpt == true) {
                        ...
                        if (skipBlock == false) {
                            ...
                            Global.computeHistogramOrder0(data.array, data.index, data.index + blockLength, histo,
                                    false);
                ...
                if (this.listeners.length > 0) {
                    ...
                    Event evt = new Event(Event.Type.AFTER_TRANSFORM, currentBlockId, postTransformLength, checksum,
                            hashType);
                    notifyListeners(this.listeners, evt);
                }

                final int bufSize = Math.max(256 * 1024,
                        Math.max(postTransformLength, blockLength + (blockLength >> 3)));
                ...
                if (this.listeners.length > 0) {
                    ...
                    Event evt = new Event(Event.Type.BEFORE_ENTROPY, currentBlockId, postTransformLength, checksum,
                            hashType);
                    notifyListeners(this.listeners, evt);
                }
                ...
                if (this.listeners.length > 0) {
                    ...
                    Event evt1 = new Event(Event.Type.AFTER_ENTROPY, currentBlockId, (written + 7) >> 3, checksum,
                            hashType);
    ...
    /**
     * A static class representing the status of a block during the compression
     * process. It encapsulates information about the block ID, the error code (if
     * any), and an optional message that provides additional details about the
     * status of the block.
     */
    static class Status {
         ...
         *
         * @param blockId
         *            The ID of the block that this status describes.
         * @param error
         *            The error code. A value of 0 indicates no error (OK), any other
         *            value indicates an error.
         * @param msg
         *            An optional message that provides additional context or details
         *            about the status of the block.
         */
    ...
    /**
     * A custom subclass of {@link ByteArrayOutputStream} that allows for using a
     * pre-existing byte buffer and ensures that the buffer is large enough to
     * accommodate the specified size.
     *
     * This class overrides the default buffer used by {@link ByteArrayOutputStream}
     * and allows for more control over the byte array used internally.
     */
    static class CustomByteArrayOutputStream extends ByteArrayOutputStream {

        /**
         * Constructs a {@link CustomByteArrayOutputStream} using the specified buffer
         * and size.
         *
         * @param buffer
         *            The byte array to use as the internal buffer. The buffer length
         *            must be greater than or equal to the specified size.
         * @param size
         *            The initial size of the output stream. This must not exceed the
         *            length of the buffer.
         *
         * @throws IllegalArgumentException
         *             If the provided buffer length is smaller than the specified size.
         */
