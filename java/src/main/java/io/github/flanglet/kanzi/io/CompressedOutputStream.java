 ...
 * listeners, and multi-threading support. It processes the data in blocks and
 * supports a variety
 ...
 * data, along with a number
 * of other internal buffers and hashers to manage the compression process
 * efficiently.
 * <p>
...
public class CompressedOutputStream extends OutputStream {
    ...
    *
    * @param os  The underlying output stream to which the compressed data will be
    *            written.
    ...
    *
    * @param obs The custom output bitstream to be used for writing compressed
    *            data.
    * @param ctx The context map that holds configuration settings for the
    *            compression process.
    ...
    * settings.
    * This method is typically called when the input buffer is full and the
    * compressed
    ...
    * entropy encoding
    * according to the specified parameters, and produces a status indicating
   ...
   static class EncodingTask implements Callable<Status> {
       ...
       * @param oBuffer          The output buffer where the encoded data will be
       *                         stored.
       ...
       * @param obs              The output bit stream used for writing the encoded
       *                         data.
       ...
       * encoding.
       * This method is executed by the {@link ExecutorService} and returns a
       * {@link Status}
       ...
       * encoding types.
       * The encoding process may involve applying a series of transforms followed by
       * entropy coding.
       *
    ...
    * process.
    * It encapsulates information about the block ID, the error code (if any), and
    * an optional
   ...
   static class Status {
       ...
       * @param error   The error code. A value of 0 indicates no error (OK), any
       *                other value indicates an error.
       * @param msg     An optional message that provides additional context or
       *                details about the status of the block.
       */
    ...
    * pre-existing byte buffer
    * and ensures that the buffer is large enough to accommodate the specified
    * size.
    *
   ...
   static class CustomByteArrayOutputStream extends ByteArrayOutputStream {
       ...
       * and size.
       *
       * @param buffer The byte array to use as the internal buffer. The buffer length
       *               must be greater than or
       *               equal to the specified size.
       * @param size   The initial size of the output stream. This must not exceed the
       *               length of the buffer.
       *
       * @throws IllegalArgumentException If the provided buffer length is smaller
